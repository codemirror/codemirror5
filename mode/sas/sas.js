...
(function (mod) {
    ...
    CodeMirror.defineMode("sas", function () {
        ...
        var define = function (style, string, context) {
            if (context) {
        ...
        // Have we found a string?
            ...
            if (word && words.hasOwnProperty(word.toLowerCase())) {
                ...
                if (/title[1-9]/i.test(word/*+stream.peek()*/)) {
                    //if (/title[1-9]/.test(word.toLowerCase()))  {
                    //stream.next();
                    return 'def';
                }
                if (word.toLowerCase() === 'footnote' && /[1-9]/.test(stream.peek())) {
                    stream.eat();
                    return 'def';
                }
                else if (word.toLowerCase() === 'footnote') {
                    return 'def';
                }
