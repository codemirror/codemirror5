...
(function(){
	  ...
	  function tokenBase(stream, state) {
		...
		if (ch == "@") {
		  ...
		  if (stream.eat("/")) {
			...
			if( /\/|\)|#/.test( stream.peek() || stream.eol() || ( stream.eatSpace() && stream.peek() == ")" ) ) )
	...
	CodeMirror.defineMode("less", lessMode);
	CodeMirror.defineMIME("text/x-less", "less");
	
	if (!CodeMirror.mimeModes.hasOwnProperty("text/css")) {
	  CodeMirror.defineMode("css", lessMode);
