  ...
  CodeMirror.simpleHint = function(editor, getHints, givenOptions) {
    ...
    function collectHints(previousToken) {
      ...
      document.body.appendChild(complete);
  		// If we're at the edge of the screen, then we want the menu to appear on the left of or above the cursor.
			var winW = window.innerWidth || Math.max( document.body.offsetWidth, document.documentElement.offsetWidth );
			if ( winW - pos.x < sel.clientWidth )
				complete.style.left = ( pos.x - sel.clientWidth ) + "px";
			var winH = window.innerHeight || Math.max( document.body.offsetHeight, document.documentElement.offsetHeight );
			if ( winH - pos.yBot < sel.clientHeight )
				complete.style.top = ( pos.y - sel.clientHeight ) + "px";
