...
// "use strict";
  function TouchSupport(cm, keyConfig) {
     ...
     document.body.appendChild(this.startSel);
     document.body.appendChild(this.endSel);
  ...
  TouchSupport.prototype = {
    ...
    *    Popup Code                                                                       */
    ...
    drawPopup : function(){
      ...
      // and display it using the cursor coordinates
      if(that.popupVisible && !cm.somethingSelected()){
        popup.appendChild(this.select);
        popup.appendChild(this.selectAll);
        if(that.clipboardText.length>0){ popup.appendChild(that.paste);}
        var middle = cm.cursorCoords().x,
            top = cm.cursorCoords().y;
      // If something is selected, fill the popup accordingly. Then derive positions for
      // selection dots from the CM.selectionDiv node and display them, taking page scrolling
      // into account. Finally, move the input field somewhere offscreen (but keep it visible),
      // since it will steal touch events on endSel due to CM's default behavior.
      } else if(cm.somethingSelected()){
        popup.appendChild(that.cut);
        popup.appendChild(that.copy);
        if(that.clipboardText.length>0){ popup.appendChild(that.paste);}
        var selectionDiv = document.getElementsByClassName('CodeMirror-selected')[0].parentNode,
            firstRect=selectionDiv.firstChild.getBoundingClientRect(),
            lastRect =selectionDiv.lastChild.getBoundingClientRect(),
            top      =firstRect.top,
            middle   =Math.min(firstRect.left, lastRect.left)+Math.max(firstRect.width, lastRect.width)/2;
        startSel.style.top  = firstRect.top + window.pageYOffset - startSel.offsetHeight/2 + "px";
        startSel.style.left = firstRect.left - startSel.offsetWidth/2 + "px";
        endSel.style.top    = lastRect.top+lastRect.height+window.pageYOffset - endSel.offsetHeight/2 + "px";
        endSel.style.left   = lastRect.left+lastRect.width - endSel.offsetHeight/2 + "px";
        cm.getInputField().style.left = '-1000px';
      }
 
      popup.style.top  = top - (popup.offsetHeight+10) + "px";
      // make sure popup doesn't go beyond left or right edge of the screen
      if(middle-popup.offsetWidth/2 < cm.getGutterElement().offsetWidth){
        popup.style.left = cm.getGutterElement().offsetWidth + "px";
        popup.classList.add('leftEdge');
      } else if(middle-popup.offsetWidth/2 > window.innerWidth-popup.offsetWidth){
 ...
 setTimer: function(e){that.timer = window.setTimeout(function(){that.showPopup(e);},800);},
    ...
    cutHandler: function(e){
      e.preventDefault();
      that.clipboardText = that.cm.getSelection().string;
