...
/*
 ...
 DoubleTap to select word (probably dependant on touch infrastructure)
  ...
  function iOSpopup(cm) {
    if(!(navigator.userAgent.match(/iPod/i) ||
     ...
     function drawPopup(e){
        ...
        // Try left, then right, then center over popupX
     ...
     function updateCursors(mode){
       return function(e){
           ...
           if(e.type=="touchend") window.clearTimeout(updateTimer);
     ...
     function selectAllHandler(){ cm.setSelection({line: 0, ch: 0}, {line: cm.lineCount() - 1}); popup.style.display = "block";}
    ...
    *    Connect Event Handlers                                                           */
     var holdTimer = null;
     var dblTapTimer = false;
     var clickTimer = null;
     function clearHoldTimer(e){
       console.log(e.type + ' - clearing hold timer');
       window.clearTimeout(holdTimer); window.clearTimeout(clickTimer);
     }
     function setHoldTimer(e){
       holdTimer = window.setTimeout(function(){magnifyCursor(e);},500);
       console.log(e.type + ' - setting hold timer');
     }
                                                     
     CodeMirror.connect(scroller,   "touchstart", setHoldTimer);
     CodeMirror.connect(scroller,   "touchmove",  clearHoldTimer);
     CodeMirror.connect(scroller,   "touchend",   clearHoldTimer);
     CodeMirror.connect(scroller,   "click",      function(e){popup.style.display=startSel.style.display=endSel.style.display='none';});
