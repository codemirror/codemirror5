  ...
  function iOSpopup(cm) {
    if(!(navigator.userAgent.match(/iPod/i) ||
     ...
     function updateSelection(mode){
       return function(e){
           // on touchEnd, show the popup only if the magnifier was previously visible. Hide the magnifier no matter what
           popupVisible = (e.type === 'touchend' && magnifier.style.display === 'block');
           ...
           var adjustY  = (mode!=="end")? startSel.firstChild.offsetHeight : -endSel.firstChild.offsetHeight,
               coords   = {x: e.touches[0].pageX, y: e.touches[0].pageY+(mode==="both"? 0 : adjustY)},
               startPos = (mode!=="end")?   cm.coordsChar(coords) : cm.getCursor(true),
               endPos   = (mode!=="start")? cm.coordsChar(coords) : cm.getCursor(false),
               scrollInfo = cm.getScrollInfo();
           ...
           if(mode==="both" || startPos.line < endPos.line || (startPos.line===endPos.line && startPos.ch<endPos.ch)){
              ...
              if((mode!=="end")){cm.scrollTo(null, Math.min(cm.cursorCoords((mode!=="end"), "local").y, scrollInfo.y));}
              if((mode!=="end")){cm.scrollTo(Math.min(cm.cursorCoords((mode!=="end"), "local").x, scrollInfo.x), null);}
              // generate magnifier at the right place, either as a cursor or selection shape
               magnifier.className = "magnifying "+(mode==="both"? "cursor" : "selection");
               magnifiedCM.scrollTo(scrollInfo.x, scrollInfo.y);
               magnifier.style.top = coords.y - Math.max(magnifier.offsetHeight, 60) + "px";
               magnifier.style.left= coords.x - magnifier.offsetWidth/2 + "px";
               magnifiedStuff.style.top  = (-coords.y+wrapper.offsetTop+magnifier.offsetHeight/4-scrollInfo.y)*magnifiedScale+"px";
               magnifiedStuff.style.left = (-coords.x+gutterWidth-scrollInfo.x-15)*magnifiedScale+magnifier.offsetWidth/2+"px";
             }
          };
     }
    ...
    function selectHandler(){
     ...
     function magnifyCursor(e){
       ...
       magnifiedCM.getScrollerElement().className = "CodeMirror-focused";
       // ensure magnifier follows the touchmove event, but hold onto that unregistration handler!
       var unregisterMove = CodeMirror.connect(scroller, "touchmove", popupFactory(updateSelection("both")), true);
       // when the finger is lifted, hide the magnifier and cleanup
       var unregisterEnd = CodeMirror.connect(scroller, "touchend", function(e){updateSelection("both")(e);
                                              unregisterMove();
                                              });

       popupFactory(updateSelection("both"))(e);
     }
    ...
    *    Connect Event Handlers                                                           */
     ...
     CodeMirror.connect(scroller,   "click",      function(){popupVisible=false; drawPopup();});
                                                   
     CodeMirror.connect(startSel,   "touchstart", popupFactory(updateSelection("start")));
     CodeMirror.connect(endSel,     "touchstart", popupFactory(updateSelection("end")));
     CodeMirror.connect(startSel,   "touchmove",  popupFactory(updateSelection("start")));
     CodeMirror.connect(endSel,     "touchmove",  popupFactory(updateSelection("end")));
     CodeMirror.connect(startSel,   "touchend",   popupFactory(updateSelection("start")));
     CodeMirror.connect(endSel,     "touchend",   popupFactory(updateSelection("end")));
