...
(function(){
 "use strict";
  function iOSpopup(cm) {
    ...
    // set style rules for CM, to make it well-behaved on iOS devices:
    // override the webkit-highlight color, give CM native scrolling, and add borders to selection elements
    var style = document.getElementById('iOSpopupCSS') || document.createElement('style');
    style.type = 'text/css';
    style.id = 'iOSselectionCSS';
    style.innerHTML = '.CodeMirror{-webkit-tap-highlight-color: rgba(0,0,0,0);}\n';
    style.innerHTML+= '.CodeMirror-scroll{  -webkit-overflow-scrolling:touch;}\n';
    style.innerHTML+= '.CodeMirror-selected:first-of-type{ border-left: solid #416BF2 2px;}\n';
    style.innerHTML+= '.CodeMirror-selected:last-of-type{ border-right: solid #416BF2 2px;}\n';
    // This one kills performance, but is necessary to prevent the textarea from stealing focus. Should be removed
    // once a better method is found
    style.innerHTML+= '.CodeMirror div textarea{position: absolute; left: -500px;}';
