     ...
     function drawTool(e){
        var startCursor = cm.cursorCoords(true),
         ...
         function drawSelectionDots(){
           startSel.style.display=(selected && startCursor.y>wrapper.offsetTop)? 'block' : 'none';
           endSel.style.display=(selected && endCursor.yBot>wrapper.offsetTop)? 'block' : 'none';
//           startSel.style.display=endSel.style.display = selected? 'block' : 'none';
        ...
        function drawPopup(){
           ...
           notch.className ='above';
           if((startCursor.y - (tool.offsetHeight)) > scrollInfo.y){
             tool.style.top  = startCursor.y - (tool.offsetHeight+10) + "px";
           } else if (endCursor.yBot+tool.offsetHeight+10 < scrollInfo.y+scroller.offsetHeight){
             notch.className='below';
             tool.style.top = endCursor.yBot+10+"px";
           } else {
             tool.style.top  = scrollInfo.y + (scroller.offsetHeight/2) + "px";
           }
