  ...
  function iOSpopup(cm) {
       ...
       return function(e){
           ...
           if(mode==="both" || startPos.line < endPos.line || (startPos.line===endPos.line && startPos.ch<endPos.ch)){
              ...
              // generate magnifier at the right place, either as a cursor or selection shape
               magnifyingGlass.className = "magnifying "+(mode==="both"? "cursor" : "selection");
               ...
               magnifiedStuff.style.top  = (-coords.y+wrapper.offsetTop+magnifyingGlass.offsetHeight/4-scrollInfo.y)*magnifiedScale+"px";
    ...
    function selectHandler(){
     ...
     function magnifyCursor(e){
       ...
       var unregister = CodeMirror.connect(scroller, "touchmove", updateSelection("both"), true);
       // when the finger is lifted, hide the magnifier and cleanuo
       CodeMirror.connect(scroller, "touchend", function(e){unregister(); updateSelection("both")(e);});
       updateSelection("both")(e);
     }
    ...
    *    Connect Event Handlers                                                           */
    ...
    if(iPad || iPhone){
       ...
       CodeMirror.connect(startSel,   "touchstart", updateSelection("start"));
       CodeMirror.connect(endSel,     "touchstart", updateSelection("end"));
       CodeMirror.connect(startSel,   "touchmove",  updateSelection("start"));
       CodeMirror.connect(endSel,     "touchmove",  updateSelection("end"));
       CodeMirror.connect(startSel,   "touchend",   updateSelection("start"));
       CodeMirror.connect(endSel,     "touchend",   updateSelection("end"));
