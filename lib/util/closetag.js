...
(function() {
	...
	CodeMirror.defaults['closeTagIndent'] = ['applet', 'blockquote', 'body', 'button', 'div', 'dl', 'fieldset', 'form', 'frameset', 'head', 'html', 'iframe', 'layer', 'legend', 'object', 'ol', 'p', 'select', 'table', 'ul'];
	...
	CodeMirror.defineExtension("closeTag", function(cm, ch, indent) {
		...
		var state = tok.state.base ? tok.state.base : tok.state;
		...
		if (state.mode == 'html') {
			...
			if (ch == '>') {
				...
				state = tok.state.base ? tok.state.base : tok.state;
