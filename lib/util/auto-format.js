(function() {
    CodeMirror.defaults['phpAutoEnabled'] = true;
    CodeMirror.defineExtension("phpAuto", function(cm, ch, indent) {
        if (!cm.getOption('phpAutoEnabled')) {
        ...
        if (ch == "(") {
            ...
            cm.replaceSelection(line + "\n");
            cm.setCursor(pos);
        } else if (ch == "CtrlShiftI") {
            alert("Start: " + info.start + "\nEnd: " + info.end + "\nString: " + info.string + "\nClass: " + info.className);
        }
    });
    CodeMirror.defineExtension("phpFormat", function(cm, action) {
        if (!cm.getOption('phpAutoEnabled')) {
            throw CodeMirror.Pass;
        }
