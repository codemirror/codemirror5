...
(function(mod) {
  ...
  function measureForScrollbars(cm) {
    var d = cm.display, scroll = d.scroller;
    if (d.gutters.style.height || d.sizer.style.minHeight || d.heightForcer.style.top)
      d.gutters.style.height = d.sizer.style.minHeight = d.heightForcer.style.top = "";
    return {
  ...
  function updateScrollbars(cm, measure) {
    ...
    var scrollHeight = Math.max(totalHeight + scrollerCutOff, measure.scrollHeight);
  ...
  function updateDisplayInner(cm, visible, forced) {
    ...
    display.gutters.style.height = display.sizer.style.minHeight = display.heightForcer.style.top = "";
  ...
  function viewCuttingPoint(cm, oldN, newN, dir) {
    ...
    while (visualLineNo(cm.doc, newN) != newN) {
      ...
      newN += view[index - (dir < 0 ? 1 : 0)].size;
      index += dir;
    }
  ...
  function leftButtonSelect(cm, e, start, type, addNew) {
    ...
    if (!addNew) {
      ourIndex = 0;
      setSelection(doc, new Selection([ourRange], 0));
    } else if (ourIndex > -1) {
      replaceOneSelection(doc, ourIndex, ourRange);
    } else {
      ourIndex = doc.sel.ranges.length;
      setSelection(doc, normalizeSelection(doc.sel.ranges.concat([ourRange]),
                                           ourIndex), false);
    }
    ...
    function extendTo(pos) {
      ...
      if (type == "rect") {
        ...
        setSelection(doc, normalizeSelection(ranges, ourIndex));
      }
    }
  ...
  function makeChangeFromHistory(doc, type, allowSelectionOnly) {
    ...
    pushPendingSelection(doc, dest, false);
    for (;;) {
      event = source.pop();
      if (event.ranges) {
        dest.push(event);
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    addSelection: docMethodOp(function(anchor, head, scroll) {
      var ranges = this.sel.ranges.slice(0);
      ranges.push(new Range(clipPos(this, anchor), clipPos(this, head || anchor)));
      setSelection(this, normalizeSelection(ranges, ranges.length - 1), scroll);
    }),
  ...
  function addSelectionToHistory(doc, sel, opId) {
    ...
    if (pend && !hist.atomicSelection && opId != hist.lastOp) {
      // A new event is started when the number of selections changed,
      // the selection moves from empty to non-empty or the other way
      // around, or when a historySelectionEventDelay has elapsed.
      var different = pend.ranges.length != sel.ranges.length ||
      ...
      // Non-overlapping, non-empty selections also start a new event.
      for (var i = 0; !different && i < sel.ranges.length; i++) {
        var selRange = sel.ranges[i], pendRange = pend.ranges[i];
        if (!selRange.empty())
          different = cmp(selRange.from(), pendRange.to()) > 0 ||
                      cmp(selRange.to(), pendRange.from()) < 0;
      }
      if (different) pushPendingSelection(doc, hist.done);
    }
  ...
  function pushPendingSelection(doc, dest, clearOpposite) {
    var hist = doc.history;
    if (hist.pendingSelection) {
      pushSelectionToHistory(hist.pendingSelection, dest);
      hist.pendingSelection = null;
      if (clearOpposite !== false)
        clearSelectionEvents(dest == hist.done ? hist.undone : hist.done);
    }
  }
