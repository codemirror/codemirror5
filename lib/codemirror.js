...
window.CodeMirror = (function() {
  ...
  function measureLineInner(cm, line) {
    ...
    for (var i = 0, cur; i < measure.length; ++i) if (cur = measure[i]) {
      ...
      if (/\bCodeMirror-widget\b/.test(cur.className) && cur.getClientRects) {
        var rects = (cur.firstChild.nodeType == 1 ? cur.firstChild : cur).getClientRects();
        var rLeft = rects[0], rRight = rects[rects.length - 1];
        if (rects.length == 1) {
          size = rLeft;
        } else {
  ...
  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    ...
    for (;;) {
      if (bidi ? to == from || to == moveVisually(lineObj, from, 1) : to - from <= 1) {
        var after = x >= fromX && x - fromX <= toX - x, ch = x < fromX || after ? from : to;
  ...
  function findPosV(cm, pos, dir, unit) {
    ...
    for (;;) {
      var target = coordsChar(cm, x - .5, y);
