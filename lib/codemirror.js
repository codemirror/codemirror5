...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    var instance = wrapper.CodeMirror = {
      ...
      getTokenAt: function(pos) {
        pos = clipPos(pos);
        return doc.get(pos.line).getTokenAt(mode, getStateBefore(pos.line), pos.ch);
      },
      ...
      coordsChar: function(coords) {
        ...
        var line = clipLine(Math.min(doc.size - 1, showingFrom + Math.floor((coords.y - off.top) / lineHeight())));
      ...
      addWidget: function(pos, node, scroll, vert, horiz) {
        ...
        if (vert == "over") top = pos.y;
          ...
          var vspace = Math.max(scroller.offsetHeight, doc.size * lineHeight()),
      ...
      getLine: function(line) {if (isLine(line)) return doc.get(line).text;},
      setLine: operation(function(line, text) {
        if (isLine(line)) replaceRange(text, {line: line, ch: 0}, {line: line, ch: doc.get(line).text.length});
      }),
    ...
    function setValue(code) {
      var top = {line: 0, ch: 0};
      updateLines(top, {line: doc.size - 1, ch: doc.get(doc.size-1).text.length},
    ...
    function unredoHelper(from, to) {
      ...
      if (change) {
        ...
        updateLinesNoUndo({line: change.start, ch: 0}, {line: end - 1, ch: doc.get(end-1).text.length}, change.old, pos, pos);
        updateInput = true;
      }
    }
    ...
    function updateLinesNoUndo(from, to, newText, selFrom, selTo) {
      ...
      var nlines = to.line - from.line, firstLine = doc.get(from.line), lastLine = doc.get(to.line);
      ...
      code.style.height = (doc.size * lineHeight() + 2 * paddingTop()) + "px";
    }
    ...
    function getRange(from, to) {
      ...
      if (l1 == l2) return doc.get(l1).text.slice(from.ch, to.ch);
      var code = [doc.get(l1).text.slice(from.ch)];
      doc.iter(l1 + 1, l2, function(line) { code.push(line.text); });
      code.push(doc.get(l2).text.slice(0, to.ch));
      return code.join("\n");
    }
    ...
    function visibleLines() {
      ...
      return {from: Math.min(doc.size, Math.max(0, Math.floor(top / lh))),
              to: Math.min(doc.size, Math.ceil((top + scroller.clientHeight) / lh))};
    }
    ...
    function refreshDisplay(from, to) {
      ...
      doc.iter(from, to, function(line) {
        ...
        html.push(line.getHTML(ch1, ch2, true));
        ++i;
      });
      ...
      for (var i = 0, e = updates.length; i < e; ++i) {
        ...
        doc.iter(rec.from, rec.to, function(line) {
          ...
          if (badInnerHTML) {
            ...
            node.innerHTML = line.getHTML(ch1, ch2, false);
    ...
    function clipPos(pos) {
      ...
      if (pos.line >= doc.size) return {line: doc.size-1, ch: doc.get(doc.size-1).text.length};
      var ch = pos.ch, linelen = doc.get(pos.line).text.length;
    ...
    function scrollEnd(top) {
      var pos = top ? {line: 0, ch: 0} : {line: doc.size - 1, ch: doc.get(doc.size-1).text.length};
      setSelectionUser(pos, pos);
    }
    function selectAll() {
      var endLine = doc.size - 1;
      setSelection({line: 0, ch: 0}, {line: endLine, ch: doc.get(endLine).text.length});
    }
    function selectWordAt(pos) {
      var line = doc.get(pos.line).text;
    ...
    function selectLine(line) {
      setSelectionUser({line: line, ch: 0}, {line: line, ch: doc.get(line).text.length});
    }
    ...
    function smartHome() {
      var firstNonWS = Math.max(0, doc.get(sel.from.line).text.search(/\S/));
    ...
    function indentLine(n, how) {
      ...
      var line = doc.get(n), curSpace = line.indentation(), curSpaceString = line.text.match(/^\s*/)[0], indentation;
      if (how == "prev") {
        if (n) indentation = doc.get(n-1).indentation();
        else indentation = 0;
      }
    ...
    function markText(from, to, className) {
      ...
      function add(line, from, to, className) {
        mark = doc.get(line).addMark(from, to, className, set);
      }
    ...
    function addGutterMarker(line, text, className) {
      if (typeof line == "number") line = doc.get(clipLine(line));
    ...
    function charX(line, pos) {
      if (pos == 0) return 0;
      measure.innerHTML = "<pre><span>" + doc.get(line).getHTML(null, null, false, pos) + "</span></pre>";
      return measure.firstChild.firstChild.offsetWidth;
    }
    function charFromX(line, x) {
      if (x <= 0) return 0;
      var lineObj = doc.get(line), text = lineObj.text;
      function getX(len) {
    ...
    function matchBrackets(autoclear) {
      var head = sel.inverted ? sel.from : sel.to, line = doc.get(head.line), pos = head.ch - 1;
      ...
      for (var i = head.line, e = forward ? Math.min(i + 100, doc.size) : Math.max(-1, i - 100); i != e; i+=d) {
        var line = doc.get(i), first = i == head.line;
    ...
    function getStateBefore(n) {
      var start = findStartLine(n), state = start && doc.get(start-1).stateAfter;
      ...
      if (n < doc.size && !doc.get(n).stateAfter) work.push(n);
      return state;
    }
    ...
    function highlightWorker() {
      ...
      while (work.length) {
        if (!doc.get(showingFrom).stateAfter) var task = showingFrom;
        else var task = work.pop();
        if (task >= doc.size) continue;
        var start = findStartLine(task), state = start && doc.get(start-1).stateAfter;
    ...
    function SearchCursor(query, pos, caseFold) {
      ...
      if (typeof query != "string") // Regexp match
        this.matches = function(reverse, pos) {
          if (reverse) {
            var line = doc.get(pos.line).text.slice(0, pos.ch), match = line.match(query), start = 0;
            while (match) {
            ...
            var line = doc.get(pos.line).text.slice(pos.ch), match = line.match(query),
        ...
        if (target.length == 1)
          this.matches = function(reverse, pos) {
            var line = fold(doc.get(pos.line).text), len = query.length, match;
          ...
          this.matches = function(reverse, pos) {
            var ln = pos.line, idx = (reverse ? target.length - 1 : 0), match = target[idx], line = fold(doc.get(ln).text);
            ...
            for (;;) {
              ...
              line = fold(doc.get(ln += reverse ? -1 : 1).text);
      ...
      find: function(reverse) {
        ...
        for (;;) {
          ...
          if (reverse) {
            ...
            pos = {line: pos.line-1, ch: doc.get(pos.line-1).text.length};
          }
          else {
  ...
  function Line(text, styles) {
    ...
    this.stateAfter = this.parent = null;
  }
  ...
  LeafChunk.prototype = {
    ...
    get: function(n) { return this.lines[n]; },
    ...
    insert: function(at, lines) {
      ...
      for (var i = 0, e = this.children.length; i < e; ++i) {
        ...
        if (at <= sz) {
          ...
          // TODO we should be in a consistent state before recursing
          this.height += (child.height - oldHeight);
