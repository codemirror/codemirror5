(function(mod) {
  ...
  function readInput(cm) {
    ...
    if (!text || text.length > 1000 || text.indexOf("\n") > -1) clearInput(cm.display);
  ...
  function clearInput(display) {
    ...
    if (!ie_lt9) {
      display.prevInput = input.value = "_";
      input.selectionStart = input.selectionEnd = 1;
      if (input.selectionEnd == 0) display.prevInput = input.value = "";
      if (old_ie) display.inputHasSelection = null;
    } else {
      display.prevInput = input.value = "";
    }
  }
    ...
    function extendTo(pos, force) {
      ...
      if (type == "rect") {
        ...
        for (var line = Math.min(start.line, pos.line), end = Math.min(cm.lastLine(), Math.max(start.line, pos.line));
          ...
          else if (text && text.length >= leftPos)
  ...
  function onContextMenu(cm, e) {
    ...
    if (!hasSelection) clearInput(cm.display);
  ...
  function findColumn(string, goal, tabSize) {
    for (var pos = 0, col = 0; col < goal && pos < string.length; pos++)
      col += string.charAt(pos) == "\t" ? tabSize - (col % tabSize) : 1;
    return pos;
  }
