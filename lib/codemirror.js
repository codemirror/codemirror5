...
window.CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    var mode, doc = new BranchChunk([new LeafChunk([new Line("", null, textHeight())])]), frontier = 0, focused;
    loadMode();
    ...
    function updateLinesNoUndo(from, to, lines, selFrom, selTo) {
      ...
      var lastHL = lines[lines.length-1], th = textHeight();
      ...
      if (from.ch == 0 && to.ch == 0 && hlText(lastHL) == "") {
        ...
        lastLine.update(lastLine.text, hlSpans(lastHL), delayedCallbacks);
        ...
        if (lines.length == 1) {
          firstLine.update(firstLine.text.slice(0, from.ch) + hlText(lines[0]) + firstLine.text.slice(to.ch),
                           hlSpans(lines[0]), delayedCallbacks);
        } else {
          for (var added = [], i = 1, e = lines.length - 1; i < e; ++i)
            added.push(new Line(hlText(lines[i]), hlSpans(lines[i]), th));
          added.push(new Line(hlText(lastHL) + firstLine.text.slice(to.ch), hlSpans(lastHL), th));
          firstLine.update(firstLine.text.slice(0, from.ch) + hlText(lines[0]), hlSpans(lines[0]), delayedCallbacks);
        ...
        firstLine.update(firstLine.text.slice(0, from.ch) + hlText(lines[0]), hlSpans(lines[0]),
                         delayedCallbacks);
        lastLine.update(hlText(lastHL) + lastLine.text.slice(to.ch), hlSpans(lastHL), delayedCallbacks);
  ...
  Line.prototype = {
    update: function(text, markedSpans, callbacks) {
      this.text = text;
      this.stateAfter = this.styles = null;
      detachMarkedSpans(this);
      attachMarkedSpans(this, markedSpans);
      signalLater(this, "change", callbacks);
    },
