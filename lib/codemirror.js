var CodeMirror = (function() {
  ...
  function Line(text) {
    this.styles = [text, null]; this.stateAfter = null; this.text = text;
  }
  Line.prototype = {
  ...
  function CodeMirror(place, options) {
    ...
    // TODO have textarea follow cursor to prevent bogus scrolling. also, wrap it in an overflow: hidden div
    div.innerHTML = '<textarea style="position: absolute; width: 10000px; left: -100000px; top: -100000px"></textarea>\
<div class="CodeMirror-code"><span style="position: absolute; visibility: hidden">-</span>\
<div style="position: relative"><div style="position: absolute; left: 0;"></div></div></div>';
    var input = div.firstChild, code = div.lastChild, measure = code.firstChild,
        space = code.lastChild, visible = space.firstChild;
    if (options.lineNumbers); // TODO
    ...
    var shiftSelecting, reducedSelection, focused, updateInput, changes;
    ...
    connect(code, "scroll", function(){updateDisplay(false);});
    ...
    function updateLines(from, to, newText, selFrom, selTo) {
      // TODO have caller do the reducing
    ...
    function unredoHelper(from, to) {
      ...
      if (change) {
        ...
        updateLines1(change.start, end, change.old, pos, pos);
      }
    }
    ...
    function updateLines1(from, to, newText, selFrom, selTo) {
      ...
      if (lendiff < 0)
        ...
        var spliceargs = [from, 0];
        for (var i = 0; i < lendiff; ++i) spliceargs.push(new Line(newText[i]));
        lines.splice.apply(lines, spliceargs);
      }
      for (var i = Math.max(0, lendiff), l = newText.length; i < l; ++i)
      ...
      // TODO function that immediately updates 'intact' array?
      changes.push({from: from, to: to, diff: lendiff});

      function updateLine(n) {return n <= Math.min(to, to + lendiff) ? n : n + lendiff;}
      showingFrom = updateLine(showingFrom);
      showingTo = updateLine(showingTo);
    ...
    function updateDisplay(scroll) {
      space.style.height = (lines.length * lineHeight()) + "px";
      if (scroll !== false) scrollCursorIntoView();
      ...
      for (var i = 0, l = changes.length; i < l; ++i) {
        var change = changes[i], intact2 = [];
        for (var j = 0, l2 = intact.length; j < l2; ++j) {
          var range = intact[j];
          if (change.to <= range.from)
            intact2.push({from: range.from + change.diff, to: range.to + change.diff, at: range.at});
            ...
            if (change.to < range.to)
              intact2.push({from: change.to + change.diff, to: range.to + change.diff,
      ...
      // TODO eventually clip off some of the nodes
      var from = Math.min(showingFrom, Math.max(visibleFrom - 2, 0)),
          to = Math.max(showingTo, Math.min(visibleTo + 2, lines.length)),
          updates = [], pos = from, at = from - showingFrom, changedLines = 0;
      if (at > 0)
      ...
      if (pos < to) {
        var size = Math.max(0, showingTo - pos);
        changedLines += size;
    ...
    function refreshDisplay(from, to) {
      ...
      for (var i = from; i < to; ++i) {
        ...
        if (inSel) {
          ch1 = 0;
          if (sel.to.line == i) {inSel = false; ch2 = sel.to.ch || null;}
        }
      ...
      visible.innerHTML = html.join("");
      showingFrom = from; showingTo = to;
      visible.style.top = (from * lineHeight()) + "px";
    }
    function patchDisplay(updates, from, to) {
      ...
      for (var i = 0, e = updates.length; i < e; ++i) {
        ...
        if (extra) {
          var nodeAfter = visible.childNodes[rec.at + off + rec.size]; 
          for (var j = Math.max(0, -extra); j > 0; --j)
            visible.removeChild(nodeAfter ? nodeAfter.previousSibling : visible.lastChild);
          for (var j = Math.max(0, extra); j > 0; --j)
            visible.insertBefore(document.createElement("div"), nodeAfter);
        }
        var node = visible.childNodes[rec.at + off], inSel = sfrom < rec.from && sto >= rec.from;
        for (var j = rec.from; j < rec.to; ++j) {
          var ch1 = null, ch2 = null;
          if (inSel) {
            ch1 = 0;
            if (sto == j) {inSel = false; ch2 = sel.to.ch || null;}
          }
          else if (sfrom == j) {
    ...
    function setSelection(from, to, oldFrom, oldTo) {
      ...
      if (!posEq(from, sel.from)) {
        if (from.line < oldFrom)
          changes.push({from: from.line, to: Math.min(to.line, oldFrom) + 1, diff: 0});
        else
          changes.push({from: oldFrom, to: Math.min(oldTo, from.line) + 1, diff: 0});
      }
      if (!posEq(to, sel.to)) {
        if (to.line < oldTo)
          changes.push({from: Math.max(oldFrom, from.line), to: oldTo + 1, diff: 0});
        else
          changes.push({from: Math.max(from.line, oldTo), to: to.line + 1, diff: 0});
      }
    ...
    function indentLine(n) {
      ...
      if (diff > 0) {
        ...
        updateLines(n, n + 1, [space + text], from, to);
      }
      else
        updateLines(n, n + 1, [text.slice(-diff)], from, to);
    }
    ...
    function replaceRange(code, from, to) {
      ...
      var end1;
      replaceRange1(code, from, to, function(end) {
        end1 = end;
    ...
    function replaceRange1(code, from, to, computeSel) {
      ...
      updateLines(from.line, to.line + 1, code, newSel.from, newSel.to);
    }
    ...
    function setLineNumbers(on) {/*TODO*/}
    function localCursorCoords(start) {
      var head = start ? sel.from : sel.to;
      return {x: head.ch * charWidth(), y: head.line * lineHeight()};
    }
    ...
    function lineHeight() {return measure.offsetHeight || 1;}
    function charWidth() {return measure.offsetWidth || 1;}

    function posFromMouse(e) {
      var off = eltOffset(space),
          x = e.pageX() - off.left,
          y = e.pageY() - off.top;
      if (e.target() == code && y < (lines.length * lineHeight())) return null;
      return clipPos({line: Math.floor(y / lineHeight()), ch: Math.round(x / charWidth())});
    }

    function findCursor() {
      if (posEq(sel.from, sel.to)) {
        if (code.getElementsByClassName) return code.getElementsByClassName("CodeMirror-cursor")[0];
        // TODO IE6 fall-back, or even rework the way the cursor works
      }
    }
    ...
    function startOperation() {
      updateInput = false; changes = [];
    }
    function endOperation() {
      if (changes.length) {updateDisplay(); restartBlink();}
      if (updateInput || changes.length)
        prepareInput();
      // TODO distinguish between text changes and sel changes
      if (changes.length && options.onCursorActivity)
        options.onCursorActivity(instance);
      if (changes.length && options.onChange)
