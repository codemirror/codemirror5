...
(function(mod) {
  ...
  function widgetMethodOp(f) {
    return function() {
      if (this.cm.curOp) return f.apply(this, arguments);
      startOperation(this.cm);
      try { return f.apply(this, arguments); }
      finally { endOperation(this.cm); }
    };
  }
  LineWidget.prototype.clear = widgetMethodOp(function() {
  ...
  LineWidget.prototype.changed = widgetMethodOp(function() {
  ...
  function addSelectionToHistory(doc, sel, opId) {
    var hist = doc.history, top = lst(hist.done);
    if (top && !top.ranges && hist.lastOp == opId) {
      hist.pendingSelection = null;
      top.selAfter = sel;
      return;
    }

    var pend = hist.pendingSelection, now = +new Date;
    if (pend && !hist.atomicSelection &&
  ...
  function pushPendingSelection(doc, dest) {
    var hist = doc.history, top = lst(dest);
    if (hist.pendingSelection && !(top && top.ranges && top.equals(hist.pendingSelection))) {
      dest.push(hist.pendingSelection);
      hist.pendingSelection = null;
    }
  }

  function startSelectionEvent(doc) {
  }
