...
window.CodeMirror = (function() {
  ...
  CodeMirror.prototype = {
    ...
    deleteH: operation(null, function(dir, unit) {
      var sel = this.doc.sel, to, deleted;
      if (!posEq(sel.from, sel.to)) to = sel.to;
      else to = findPosH(this.doc, sel.head, dir, unit, false);
      if (posLess(sel.from, to))
        deleted = this.getRange(sel.from, to);
      else
        deleted = this.getRange(to, sel.from);
      replaceRange(this.doc, "", sel.from, to, "+delete");
  ...
  var commands = CodeMirror.commands = {
    ...
    killLine: function(cm) {
      var from = cm.getCursor(true), to = cm.getCursor(false), sel = !posEq(from, to), killed = null;
      if (!sel && cm.getLine(from.line).length == from.ch)
        to = Pos(from.line + 1, 0);
      else
        to = sel ? to : Pos(from.line);
      killed = cm.getRange(from, to);
