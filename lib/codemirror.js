var CodeMirror = (function() {
  ...
  function Line(text, styles) {
    this.styles = styles || [text, null]; this.stateAfter = null; this.text = text;
  }
  Line.prototype = {
...
<div style="position: relative"><div style="position: absolute; left: 0;"></div></div></div>';
    ...
    var shiftSelecting, reducedSelection, focused, updateInput, changes, textChanged;
    ...
    function readInput() {
      ...
      if (changed) {
        ...
        var ch = nl > -1 ? start - nl : start, endline = editing.to - 1;
        for (var edend = editing.text.length;
             edend > start && end > start && (c = text.charAt(end)) == editing.text.charAt(edend);
             edend--, end--)
          if (c == "\n") endline--;
        var nl = text.lastIndexOf("\n", edend), endch = nl == -1 ? edend : edend - nl - 1;
    ...
    function refreshDisplay(from, to) {
      ...
      for (var i = from; i < to; ++i) {
        ...
        if (inSel) {
          ch1 = 0;
          if (sel.to.line == i) {inSel = false; ch2 = sel.to.ch || (ch1 = null);}
        }
    ...
    function patchDisplay(updates, from, to) {
      ...
      for (var i = 0, e = updates.length; i < e; ++i) {
        ...
        for (var j = rec.from; j < rec.to; ++j) {
          ...
          if (inSel) {
            ch1 = 0;
            if (sto == j) {inSel = false; ch2 = sel.to.ch || (ch1 = null);
            }
          }
          else if (sfrom == j) {
    ...
    function startOperation() {
      updateInput = null; changes = []; textChanged = false;
    }
    function endOperation() {
      ...
      if (updateInput === true || (updateInput !== false && changes.length))
        prepareInput();
