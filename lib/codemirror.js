var CodeMirror = (function() {
  ...
  function CodeMirror(place, options) {
    ...
    div.innerHTML = '<div class="CodeMirror-code">' +
      ...
      '<div style="position: relative"><pre style="position: absolute; visibility: hidden"><span>-</span></pre>' +
      '<div style="overflow: hidden; position: absolute; width: 0">' + 
      '<textarea style="position: absolute; width: 10000px;"></textarea></div>' +
      '<pre class="CodeMirror-cursor">&nbsp;</pre><div class="CodeMirror-lines"></div></div></div></div>';
...
//    inputDiv.style.width = "30em"; inputDiv.style.height = "5em"; input.rows = 3;
    ...
    function onKeyDown(e) {
      ...
      if (code == 33 || code == 34) {scrollPage(code == 34); done = true;} // page up/down
        ...
        else if (!ctrl && code == 9) {if (handleTab(e.e.shiftKey)) done = true;} // tab
        else if (ctrl && code == 90) {undo(); done = true;} // ctrl-z
        else if (ctrl && ((e.e.shiftKey && code == 90) || code == 89)) {redo(); done = true;} // ctrl-shift-z, ctrl-y
      }
    ...
    function updateDisplay(changes) {
      ...
      var visibleTo = Math.min(lines.length, Math.ceil((top + code.clientHeight) / lh));
      ...
      if (lineDiv.childNodes.length != showingTo - showingFrom)
        throw new Error("BAD! " + JSON.stringify(updates) + " size=" + (showingTo - showingFrom) + " nodes=" + lineDiv.childNodes.length);
    }
    ...
    function updateGutter() {
      ...
      for (var i = showingFrom; i < showingTo; ++i) {
        ...
        if (marker) html.push('<pre class="' + marker.style + '">' + htmlEscape(marker.text || "\u00a0") + '</pre>');
        else html.push("<pre>" + (options.lineNumbers ? i + options.firstLineNumber : "\u00a0") + "</pre>");
      }
    ...
    function indentLine(n, mode) {
      if (mode == "smart") {
        if (!parser.indent) mode = "prev";
        else {
      ...
      else if (mode == "smart") indentation = parser.indent(state, line.text.slice(curSpace));
      else if (mode == "add") indentation = curSpace + options.indentUnit;
      else if (mode == "subtract") indentation = Math.max(0, curSpace - options.indentUnit);
    ...
    function setParser(spec) {
      options.parser = spec;
      parser = CodeMirror.getParser(options, spec);
      for (var i = 0, l = lines.length; i < l; ++i)
  ...
  CodeMirror.getParser = function(options, spec) {
    if (typeof spec == "string")
      var pname = spec, config = {};
    else
      var pname = spec.name, config = spec;
    var pfactory = parsers[pname];
    if (!pfactory) throw new Error("No parser " + pname + " found.");
    return pfactory(options, config);
  }
