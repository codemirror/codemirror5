...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    var instance = wrapper.CodeMirror = {
      ...
      setOption: function(option, value) {
        ...
        else if (option == "customScrollSpeed"){
  	      var type = gecko ? "DOMMouseScroll" : "mousewheel";
		      !omw && value ? omw = connect(scroller, type, onMouseWheel, true) : (omw(),omw=null);
		    }
    ...
    function onMouseWheel(e) {  	
      ...
      if (y) {
        var ny = y/30 * options.scrollSpeed;	
        scrollbar.scrollTop += ny;
      }
      if ( ny ) e_stop(e);
    }
    ...
    function patchDisplay(from, to, intact) {
      ...
      doc.iter(from, to, function(line) {
        ...
        if (!nextIntact || nextIntact.from > j) {
          if (line.hidden) var lineElement = document.createElement("pre");
          else {
            var lineElement = line.getElement(makeTab);
            if (line.className) lineElement.className = line.className;
            // Kludge to make sure the styled element lies behind the selection (by z-index)
            if (line.bgClassName) {
              var div = createElement("div", null, null, "position: relative");
              var pre = createChild(div, "pre", "\u00a0", line.bgClassName, "position: absolute; left: 0; right: 0; top: 0; bottom: 0; z-index: -2");
      ...
      doc.iter(showingFrom, Math.max(showingTo, showingFrom + 1), function(line) {
        if (line.hidden) {
          ...
          if (marker && marker.text)
            ...
            text = "\u00a0";
          var markerElement = createChild(fragment, "pre", null, marker && marker.style);
          markerElement.innerHTML = text;
          for (var j = 1; j < line.height; ++j) {
            createChild(markerElement, "br");
      ...
      if (collapsed) {
        ...
        var add = function(left, top, right, height) {
          var rstyle = quirksMode ? "width: " + (!right ? clientWidth : clientWidth - right - left) + "px"
                                  : "right: " + right + "px";
          createChild(fragment, "div", null, "CodeMirror-selected", "position: absolute; left: " + left +
    ...
    function makeTab(col) {
      ...
      var span = createElement("span", str, "cm-tab");
    ...
    function coordsChar(x, y) {
      ...
      for (;;) {
        if (to - from <= 1) {
          ...
          return {line: lineNo, ch: (Math.max(dright, dleft) > cw * 2 || dleft < dright) && ( (x-toX)+(toX/to) < (toX/to)/2 ) ? from : to};
        }
        var middle = Math.ceil((from + to) / 2), middleX = getX(middle);
        if (middleX > x) {to = middle; toX = middleX;}
        else {from = middle; fromX = middleX;}
    ...
    function textHeight() {
      if (measurePre == null) {
        measurePre = document.createElement("pre");
        for (var i = 0; i < 49; ++i) {
          measurePre.appendChild(document.createTextNode("x"));
          createChild(measurePre, "br");
        }
        measurePre.appendChild(document.createTextNode("x"));
      }
    ...
    function charWidth() {
      ...
      var anchor = createChild(pre, "span", "x");
      removeChildrenAndAdd(measure, pre);
  ...
  CodeMirror.fromTextArea = function(textarea, options) {
    ...
    //check for customScrollSpeed
    if(CodeMirror.defaults.customScrollSpeed || options.customScrollSpeed)instance.setOption('customScrollSpeed',true);
    return instance;
  };
        ...
        if (!specials.test(text)) {
          ...
          while (true) {
            ...
            if (m[0] == "\t") {
              ...
              var token = createElement("span", "\u2022", "cm-invalidchar");
        ...
        span = function(html, text, style) {
          ...
          if (wrapAt >= outPos && wrapAt < outPos + l) {
            if (wrapAt > outPos) {
              ...
              if (wrapWBR) html.appendChild(document.createElement("wbr"));
            }
            html.appendChild(open);
            var cut = wrapAt - outPos;
            span_(open, opera ? text.slice(cut, cut + 1) : text.slice(cut), style);
            ...
            if (outPos == wrapAt && outPos == len) {
              setTextContent(open, eolSpanContent);
              html.appendChild(open);
            }
  ...
  function createChild(parent, tag, text, className, style) {
    var e = createElement(tag, text, className, style);
  ...
  function isWordChar(ch) {
    return /^[\/<>.,;:(){}\[\]!$%^&*_\-\\?=+\|#'~`]/.test(ch) || ch.toUpperCase() != ch.toLowerCase();
  }
