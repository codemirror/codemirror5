...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
      ...
      doc.iter(showingFrom, Math.max(showingTo, showingFrom + 1), function(line) {
        if (line.hidden) {
          ...
          var marker = line.gutterMarker,
              markerElement = fragment.appendChild(elt("pre", null, line.gutterClassName)),
              text = options.lineNumbers ? options.lineNumberFormatter(i + options.firstLineNumber) : null;
          if (marker) {
            if (marker.nodeType) {
              marker = marker.cloneNode(true);
              marker.textContent = marker.textContent.replace("%N%", text != null ? text : "");
              markerElement.appendChild(marker);
            } else markerElement.innerHTML = marker.replace("%N%", text != null ? text : "");
          } else markerElement.appendChild(document.createTextNode(text != null ? text : "\u00a0"));
    ...
    function addGutterMarker(line, marker, className) {
      if (typeof line == "number") line = getLine(clipLine(line));
      line.gutterMarker = marker;
      line.gutterClassName = className;
      gutterDirty = true;
      return line;
    }
    function removeGutterMarker(line) {
      if (typeof line == "number") line = getLine(clipLine(line));
      delete line.gutterMarker;
      delete line.gutterClassName;
      gutterDirty = true;
    }
    ...
    function lineInfo(line) {
      ...
      return {line: n, handle: line, text: line.text, marker: line.gutterMarker,
              markerClass: line.gutterClassName, lineClass: line.className,
