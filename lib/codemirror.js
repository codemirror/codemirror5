...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    wrapper.className = "CodeMirror" + (options.textWrapping ? " CodeMirror-wrap" : "");
    ...
    var instance = wrapper.CodeMirror = {
      ...
      setOption: function(option, value) {
        ...
        else if (option == "textWrapping" && oldVal != value) operation(wrappingChanged)();
      },
    ...
    function scrollCursorIntoView() {
      ...
      var x = options.textWrapping ? Math.min(cursor.x, lineSpace.clientWidth) : cursor.x;
    ...
    function updateCursor() {
      ...
      if (posEq(sel.from, sel.to)) {
        ...
        cursor.style.left = (options.textWrapping ? Math.min(pos.x, lineSpace.clientWidth) : pos.x) + "px";
    ...
    function setSelection(from, to, oldFrom, oldTo) {
      ...
      function skipHidden(pos, oldLine) {
        var line = getLine(pos.line);
        if (!line.hidden) return pos;
        if (pos.line > oldLine) return getNonHidden(pos, 1) || getNonHidden(pos, -1);
        else return getNonHidden(pos, -1) || getNonHidden(pos, 1);
      }
      function getNonHidden(pos, dir) {
        var lNo = pos.line + dir, end = dir == 1 ? doc.size : -1;
        while (lNo != end) {
          var line = getLine(lNo);
          if (!line.hidden) return {line: lNo, ch: Math.min(line.text.length, pos.ch)};
          lNo += dir;
        }
      }
      if (from.line != oldFrom) from = skipHidden(from, oldFrom);
      if (to.line != oldTo) to = skipHidden(to, oldTo);
    ...
    function setLineClass(handle, className) {
      return changeLine(handle, function(line) {
        if (line.className != className) {
          found.line.className = className;
          return true;
        }
      });
    }
    ...
    function measureLine(line, ch) {
      // The extra text at the end is added to make sure the line is wrapped in the right way.
      measure.innerHTML = "<pre>" + line.getHTML(null, null, false, ch) +
        '<span id="CodeMirror-temp"></span>' + line.text.slice(ch, ch + 30) + "</pre>";
    ...
    function localCoords(pos, inLineWrap) {
      ...
      if (pos.ch == 0) x = 0;
        ...
        x = sp.offsetLeft;
        if (options.textWrapping) y += Math.max(0, sp.offsetTop);
      }
    ...
    function coordsChar(x, y) {
      ...
      var tw = options.textWrapping, innerOff = tw ? heightPos - heightAtLine(doc, lineNo) : 0;
      ...
      var from = 0, fromX = 0, to = text.length, toX;
      // Guess a suitable upper bound for our search.
      var estimated = Math.min(to, Math.ceil(x / cw));
      for (;;) {
        var estX = getX(estimated);
        if (estX <= x && estimated < to) estimated = Math.min(to, Math.ceil(estimated * 1.2));
        else {toX = estX; to = estimated; break;}
