...
// TODO: vertical mousewheel/touch scrolling might not work in all browsers
// TODO: horizontal scrolling is wonky in main Brackets app (but works ok in browser)
// ISSUE: current way of laying out scrollbar might break existing CodeMirror users, because
//       it's explicitly documented that you should set the editor width by directly styling
//       .CodeMirror-scroll -- should consider having an API for this instead
...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
        ...
        '<div style="position: relative">' + // Set to the height of the text, causes scrolling
          '<div style="position: relative">' + // Moved around its parent to cover visible view
            ...
            '<div class="CodeMirror-lines"><div style="position: relative">' +
              ...
              '<div style="position: absolute; z-index:-1;"></div>' + // This DIV contains the selection
    ...
    function cloneWheelEvent(e, deltaX, deltaY) {
      var clone = document.createEvent("WheelEvent");
      if (clone.initWebKitWheelEvent) {
        // WebKit browsers don't support initWheelEvent yet.
        clone.initWebKitWheelEvent(deltaX, deltaY, e.view, 
          e.screenX, e.screenY, e.pageX, e.pageY,
          e.ctrlKey, e.altKey, e.shiftKey, e.metaKey);
      }
      else {
        // TODO: handle other older browsers that don't support WheelEvent
        clone.initWheelEvent(e.type, e.bubbles, e.cancelable, e.view, e.detail, 
          e.screenX, e.screenY, e.clientX, e.clientY, 
          e.ctrlKey, e.altKey, e.shiftKey, e.metaKey, e.button, e.relatedTarget,
          deltaX, deltaY, e.deltaZ, e.deltaMode);
      } 
      return clone;     
    }
    
    function onMouseWheel(e) {
      // Forward the vertical component of the event to the vertical scrollbar, but let the
      // underlying scroller process the horizontal component, if any (it will ignore the
      // vertical component).
      if (e.deltaY || e.wheelDeltaY) {
        scrollbar.dispatchEvent(cloneWheelEvent(e, 0, e.deltaY || e.wheelDeltaY));
      }
    ...
    function updateVerticalScroll() {
      // TODO: simulate overflow: auto by hiding scrollbar if content isn't overflowing?
      var th = textHeight();
      scrollbar.style.height = scroller.clientHeight + "px";
      scrollbarInner.style.height = (doc.height * th + 2 * paddingTop()) + "px";
    ...
    function updateDisplay(changes, suppressCallback) {
      ...
      // TODO: does this need to happen after line heights are updated in the lineWrapping case below?
      updateVerticalScroll();
    ...
    function updateSelection() {
      ...
      if (!posEq(sel.from, sel.to)) {
        // TODO - pin range to visible area?
        if (sel.from.line == sel.to.line) { // Single line selection
          selectionDiv.innerHTML = getSelectionHTML(sel.from, sel.to, false);
        } else { // Multi line selection
          var html = [];
          // First line
          html.push(getSelectionHTML(sel.from, sel.from, true));
          // Last line
          html.push(getSelectionHTML({line: sel.to.line, ch: 0}, sel.to, false));
          // Middle block
          if (sel.to.line > sel.from.line + 1)
            html.push(getSelectionHTML({line: sel.from.line + 1, ch: 0}, {line: sel.to.line - 1, ch: 0}, true));
          selectionDiv.innerHTML = html.join("");
        }
      }
    }
    function getSelectionHTML(from, to, padToEnd) {
      var fromPos = localCoords(from, true), toPos = localCoords(to, true), 
          viewWidth = scroller.clientWidth - (eltOffset(lineSpace, false).left - eltOffset(scroller, false).left),
          width = padToEnd ? (Math.max(lineDiv.clientWidth, viewWidth) - fromPos.x) : (toPos.x - fromPos.x);
      var html = [];
      html.push("<span class='CodeMirror-selected' style='position:absolute;");
        html.push("left:" + fromPos.x + "px;");
        html.push("top:" + fromPos.y + "px;");
        html.push("width:" + width + "px;");
        html.push("height:" + (toPos.yBot - fromPos.y) + "px;");
      html.push("'></span>");
      return html.join("");
    }
