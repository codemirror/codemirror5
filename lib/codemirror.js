...
window.CodeMirror = (function() {
  ...
  function updateDisplayInner(cm, changes, viewPort) {
    ...
    for (var node = display.lineDiv.firstChild; node; node = node.nextSibling) if (node.lineObj) {
  ...
  function patchDisplay(cm, from, to, intact, updateNumbersFrom) {
    ...
    if (!intact.length && !ie && (!safari || !cm.display.currentWheelTarget))
      removeChildren(display.lineDiv);
    var container = display.lineDiv, cur = container.firstChild;

    function rm(node) {
      var next = node.nextSibling;
      if (safari && mac && cm.display.currentWheelTarget == node) {
  ...
  function onScrollWheel(cm, e) {
    ...
    if (dy && wheelPixelsPerUnit != null) {
      ...
      if (mac && safari) for (var cur = e.target; cur != scroll; cur = cur.parentNode) {
        if (cur.lineObj) {
