...
window.CodeMirror = (function() {
  ...
  function CodeMirror(place, options) {
    ...
    if (options.gutterOnRight)
      display.gutters.style.left = display.sizer.offsetWidth + "px";
  ...
  function alignHorizontally(cm) {
    ...
    var l = comp + (cm.options.gutterOnRight ? display.sizer.offsetWidth : 0) + "px";
    for (var n = display.lineDiv.firstChild; n; n = n.nextSibling) if (n.alignable) {
      for (var i = 0, a = n.alignable; i < a.length; ++i) a[i].style.left = l;
    }
    if (cm.options.fixedGutter) {
      if (cm.options.gutterOnRight)
        display.gutters.style.left = display.sizer.offsetWidth + "px";
      else display.gutters.style.left = (comp + gutterW) + "px";
    }
  }
  ...
  function updateDisplayInner(cm, changes, visible) {
    ...
    if (cm.getOption("gutterOnRight"))
      var gutterW = display.sizer.style.marginRight =
          display.gutters.offsetWidth + scrollbarWidth(display.measure) + "px";
    else var gutterW = display.sizer.style.marginLeft = display.gutters.offsetWidth + "px";
    ...
    if (cm.options.lineNumbers || markers) {
      var gutterWrap = wrap.insertBefore(elt("div", null, null, "position: absolute; left: " +
                                             ((cm.options.fixedGutter ? dims.fixedPos : - dims.gutterTotalWidth) +
                                              (cm.options.gutterOnRight ? cm.display.sizer.offsetWidth : 0)) + "px"),
                                         wrap.firstChild);
  ...
  function clickInGutter(cm, e) {
    ...
    if (cm.options.gutterOnRight) {
      if (mX <= Math.ceil(getRect(display.gutters).left)) return false;
    } else {
  ...
  option("fixedGutter", true, function(cm, val) {
    var base = cm.options.gutterOnRight ? display.sizer.offsetWidth : 0;
    cm.display.gutters.style.left = val ? (compensateForHScroll(cm.display) + base) + "px" : base + "px";
    cm.refresh();
  }, true);
