var CodeMirror = (function() {
  ...
  CodeMirror.prototype = {
    ...
    displaySelection: function() {
      ...
      function nAfter(n) {
        return n.nextSibling || headLine.appendChild(document.createElement("span"));
      }
      if (sel.inverted) {
        // TODO proper use of offset properties
        for (var n = headLine.lastChild; n; n = n.previousSibling) {
          if (/\bCodeMirror-(?:selected|cursor)/.test(n.className)) {
            if (/\bCodeMirror-cursor-right\b/.test(n.className)) n = nAfter(n);
            break;
          }
        }
      }
      else {
        for (var n = headLine.firstChild; n; n = n.nextSibling) {
          if (/\bCodeMirror-(?:selected|cursor)/.test(n.className)) {
            if (!/\bCodeMirror-cursor-left\b/.test(n.className))
              n = nAfter(n);
            break;
          }
        }
      }
      var ytop = (n || headLine).offsetTop, xleft = (n || headLine).offsetLeft;

      var screen = this.code.clientHeight, screentop = this.code.scrollTop;
      if (ytop < screentop)
        this.code.scrollTop = Math.max(0, ytop - 10);
      else if ((ytop += this.lineHeight()) > screentop + screen)
        this.code.scrollTop = (ytop + 10) - screen;

      var screenw = headLine.offsetWidth, screenleft = this.code.scrollLeft;
      if (xleft < screenleft)
        this.code.scrollLeft = Math.max(0, xleft - 10);
      else if (xleft > screenw + screenleft)
        this.code.scrollLeft = (xleft + 10) - screenw;
    },
    ...
    findCursor: function() {
      ...
      if (posEq(sel.from, sel.to)) {
        var div = this.lines[sel.from.line].div;
        var left = div.getElementsByClassName("CodeMirror-cursor-left")[0];
        if (left) return {node: left, side: "left"};
        var right = div.getElementsByClassName("CodeMirror-cursor-right")[0];
        if (right) return {node: right, side: "right"};
      }
    },
    restartBlink: function() {
      clearInterval(this.blinker);
      var on = true, self = this, cur;
      this.blinker = setInterval(function() {
        cur = self.findCursor() || cur;
        if (cur) {
          if (on = !on) cur.node.className += " CodeMirror-cursor-" + cur.side;
          else cur.node.className = cur.node.className.replace(/ \bCodeMirror-cursor-\w+\b/, "");
        }
      }, 650);
    },
