      ...
      doc.iter(showingFrom, Math.max(showingTo, showingFrom + 1), function(line) {
        if (line.hidden) {
          ...
          if (marker && marker.text)
            ...
            text = "\u00a0";
          var markerElement = createChild(fragment, "pre", text, marker && marker.style);
    ...
    getElement: function(makeTab, wrapAt, wrapWBR) {
      ...
      function span_(html, text, style) {
        ...
        if (!specials.test(text)) {
          col += text.length;
          var escaped = document.createTextNode(text);
        } else {
          var escaped = document.createDocumentFragment();
          for (var pos = 0; pos < text.length; ++pos) {
            var ch = text.charAt(pos);
            if (ch == "\t") {
              var tab = makeTab(col);
              escaped.appendChild(tab.element.cloneNode(true));
              col += tab.width;
            } else {
              col += 1;
              if (specials.test(ch)) {
                ...
                escaped.appendChild(token);
              } else escaped.appendChild(document.createTextNode(ch));
            }
          }
        }
        if (style) {
          ...
          token.appendChild(escaped);
          html.appendChild(token);
        } else html.appendChild(escaped);
      }
      var span = span_;
      if (wrapAt != null) {
        var outPos = 0;
