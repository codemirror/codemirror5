...
<div style="position: relative"><span class="CodeMirror-cursor"> </span><div style="position: absolute; left: 0;"></div></div></div>';
    var input = div.firstChild, code = div.lastChild, measure = code.firstChild,
        space = code.lastChild, cursor = space.firstChild, visible = cursor.nextSibling;
    ...
    function setSelection(from, to, oldFrom, oldTo) {
      ...
      if (posEq(from, to)) {
        if (!posEq(sel.from, sel.to))
          changes.push({from: oldFrom, to: oldTo + 1, diff: 0});
      }
      else if (posEq(sel.from, sel.to)) {
        changes.push({from: from.line, to: to.line + 1, diff: 0});
      }
      else {
    ...
    function updateCursor() {
      if (posEq(sel.from, sel.to)) {
        cursor.style.top = sel.from.line * lineHeight() + "px";
        cursor.style.left = sel.from.ch * charWidth() + "px";
