...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    // endOperation() uses this to determine if the cursor should be scrolled into view. startOperation()
    // sets this to true (the default), so a command that wants to change the default behavior should change
    // this variable after the call to startOperation.
    var scrollToCursorOnSelChange;
    ...
    var instance = wrapper.CodeMirror = {
      ...
      setSelection: operation(function(from, to, user, scrollToCursor) {
        if( scrollToCursor === undefined ) scrollToCursorOnSelChange = true;
        else scrollToCursorOnSelChange = scrollToCursor;
    ...
    function startOperation() {
      ...
      changes = []; selectionChanged = false; scrollToCursorOnSelChange = true; callbacks = [];
    }
    function endOperation() {
      var reScroll = false, updated;
      if (selectionChanged && scrollToCursorOnSelChange) reScroll = !scrollCursorIntoView();
      if (changes.length) updated = updateDisplay(changes, true);
      else {
        if (selectionChanged) updateCursor();
        if (gutterDirty) updateGutter();
      }
      if (reScroll && scrollToCursorOnSelChange) scrollCursorIntoView();
  ...
  var commands = CodeMirror.commands = {
    selectAll: function(cm) {cm.setSelection({line: 0, ch: 0}, {line: cm.lineCount() - 1}, false, false);},
    ...
    goDocEnd: function(cm) {cm.setSelection({line: cm.lineCount() - 1}, null, true, true);},
    ...
    goLineEnd: function(cm) {cm.setSelection({line: cm.getCursor().line}, null, true, true);},
