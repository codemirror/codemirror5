...
window.CodeMirror = (function() {
  ...
  function updateDocNoUndo(cm, from, to, lines, selFrom, selTo) {
    ...
    setSelection(cm, clipPos(doc, selFrom), clipPos(doc, selTo), null, true);
  }
  ...
  function setSelection(cm, from, to, bias, checkAtomic) {
    ...
    if (checkAtomic || !posEq(from, sel.from))
      from = skipAtomic(cm, from, bias);
    if (checkAtomic || !posEq(to, sel.to))
      to = skipAtomic(cm, to, bias);
  ...
  function reCheckSelection(cm) {
    setSelection(cm, cm.view.sel.from, cm.view.sel.to, null, true);
  }

  function skipAtomic(cm, pos, bias) {
    var curLine = pos.line, curCh = pos.ch;
    var doc = cm.view.doc, line = getLine(doc, curLine);
    var forward = bias != -1, flipped = false;
    search: for (;;) {
      while (line.hidden && line.hidden.handle.unfoldOnEnter)
  ...
  function findPosH(cm, dir, unit, visually) {
    ...
    return skipAtomic(cm, {line: line, ch: ch}, dir);
  }
