...
window.CodeMirror = (function() {
  ...
  var commands = CodeMirror.commands = {
    ...
    moveLinesUp: function(cm) {cm.moveSelectedLines(-1);},
    moveLinesDown: function(cm) {cm.moveSelectedLines(1);},
  ...
  keyMap.pcDefault = {
    ...
    "Alt-Up": "moveLinesUp", "Alt-Down": "moveLinesDown",
    fallthrough: "basic"
  };
  keyMap.macDefault = {
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    moveLines: function(start, end, offset) {  // Moves a block of lines by an offset.
      if (offset == 0 || start > end) {return 0;}
      // Clipping the offset so that the move wouldn't go beyond the first or the last line.
      if (start + offset < this.firstLine()) {offset = this.firstLine() - start;}
      else if (end + offset > this.lastLine()) {offset = this.lastLine() - end;}
      // Copying the selected lines.
      var selectedLines = [];
      for (var line = start; line <= end; line++) {selectedLines.push(this.getLine(line));}
      ...
      if (offset > 0) {  // Shifting lines below up.
        for (var line = end + 1; line <= end + offset; line++) {
          this.setLine(line - selectedLines.length, this.getLine(line));
        }
      ...
      for (var line = start; line <= end; line++) {
        this.setLine(line + offset, selectedLines[line - start]);
      }
      return offset;
    },
    moveSelectedLines: function(offset) {  // Moves selected lines with selection by an offset.
      if (offset == 0) {return 0;}
      var cursorStart = this.getCursor('start');
      var cursorEnd = this.getCursor('end');
      var cursor = this.getCursor();
      offset = this.moveLines(cursorStart.line, cursorEnd.line, offset);
      // Calculating type of selection: left-to-right or right-to-left.
      var leftToRightSelection = (cursor.line == cursorEnd.line
          && cursor.ch == cursorEnd.ch);  // Else right-to-left selection.
      // Moving the selection.
      cursorStart.line += offset;
      cursorEnd.line += offset;
      if (leftToRightSelection) {this.setSelection(cursorStart, cursorEnd);}
      else {this.setSelection(cursorEnd, cursorStart);}
      return offset;
    },
