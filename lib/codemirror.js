...
window.CodeMirror = (function() {
  ...
  function lineLength(doc, line) {
    ...
    while (merged = collapsedSpanAt(cur, 0)) {
  ...
  function updateDisplayInner(cm, changes, scrollTop) {
    ...
    if (changes !== true && sawCollapsedSpans)
      for (var i = 0; i < changes.length; ++i) {
        ...
        while (merged = collapsedSpanAt(getLine(doc, ch.from), 0)) {
    ...
    if (sel.from.line == sel.to.line) {
      ...
      if (singleLine) {
        ...
        if (!sel.to.ch)
          ...
          middleBot = drawForLine(sel.to.line, collapsedSpanAt(toObj, 0) ? null : 0, sel.to.ch, true);
  ...
  function updateDocNoUndo(cm, from, to, lines, selUpdate) {
    ...
    if (!cm.options.lineWrapping) {
      for (var cur = firstLine, merged; merged = collapsedSpanAt(cur, 0);) {
  ...
  function markText(cm, from, to, options, type) {
    ...
    doc.iter(curLine, to.line + 1, function(line) {
      ...
      if (marker.collapsed) {
        ...
        if (curLine != from.line) updateLineHeight(line, 0);
      }
  ...
  function makeLine(text, markedSpans, height) {
    ...
    if (markedSpans && collapsedSpanAt(line, 0)) line.height = 0;
    return line;
  }
  ...
  function updateLine(cm, line, text, markedSpans) {
    ...
    attachMarkedSpans(line, markedSpans);
    var hidden = collapsedSpanAt(line, 0);
  ...
  function lineContent(cm, line, anchorAt) {
    var merged;
    while (merged = collapsedSpanAt(line, 0)) {
      ...
      if (anchorAt < builder.pos + l && (anchorAt >= builder.pos || !builder.pre.anchor)) {
        ...
        if (cut + 1 == l) {
          ...
          if (cut < 0) {
            ...
            var last = builder.pre.lastChild, widgetBefore = /\bCodeMirror-widget\b/.test(last.className);
  ...
  function heightAtLine(cm, lineObj) {
    var merged;
    while (merged = collapsedSpanAt(lineObj, 0))
  ...
  function lineStart(cm, lineNo) {
    var merged, line;
    while (merged = collapsedSpanAt(line = getLine(cm.view.doc, lineNo), 0))
