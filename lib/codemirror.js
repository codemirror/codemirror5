...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    else if (/MSIE 7\b/.test(navigator.userAgent)) {
    ...
    var updateInput, userSelChange, changes, textChanged, selectionChanged, leaveInputAlone,
        gutterDirty, callbacks, lineHeightChanged;
    ...
    function updateLineHeight(line, height) {
      gutterDirty = lineHeightChanged = true;
    ...
    function scrollIntoView(x1, y1, x2, y2) {
      ...
      if (y1 < screentop) {scrollbar.scrollTop = Math.max(0, y1); scrolled = true;}
      else if (y2 > screentop + screen) {scrollbar.scrollTop = y2 - screen; scrolled = true;}
    ...
    function startOperation() {
      ...
      changes = []; selectionChanged = lineHeightChanged = false; callbacks = [];
    }
    function endOperation() {
      var reScroll = false, updated;
      if (selectionChanged && !lineHeightChanged) reScroll = !scrollCursorIntoView();
      if (changes.length) updated = updateDisplay(changes, true);
      else {
        if (selectionChanged) updateSelection();
        if (gutterDirty) updateGutter();
      }
      if (reScroll || lineHeightChanged) scrollCursorIntoView();
