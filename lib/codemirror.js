...
//    te.style.left = te.style.top = "-100000px";
    ...
    var self = this;
    connect(div, "mousedown", function(e){self.onMouseDown(e);});
    connect(te, "keyup", function(e){self.onKeyUp(e);});
    connect(te, "keydown", function(e){self.onKeyDown(e);});
    ...
    connect(div, "drop", function(e){self.onDrop(e);});
    connect(div, "paste", function(e){self.input.focus(); self.schedulePoll(20);});
  ...
  CodeMirror.prototype = {
    ...
    onMouseDown: function(e) {
      ...
      var leave = connect(this.div, "mouseout", function(e) {
    ...
    schedulePoll: function(time, keyId) {
      ...
      if (time) this.pollTimer = setTimeout(keyId ? pollForKey : poll, time);
    },
    ...
    readInput: function() {
      ...
      var lineCrossed = rs || from.line != sel.from.line || to.line != sel.to.line || from.line != to.line;

      if (changed) {
        this.shiftSelecting = null;
        var rpl = this.replaceLines(ed.from, ed.to, text.split(/\r?\n/g));
        if (rpl.from != sel.from.line || rpl.to != sel.from.line + 1 || rpl.diff) lineCrossed = true;
      }

      if (!lineCrossed) {ed.start = selstart; ed.end = selend; ed.text = text;}
      this.setSelection(from, to, lineCrossed);
    ...
    displaySelection: function() {
      ...
      var ypos = lineElt(this.lines[sel.from.line]).offsetTop, line = this.lineHeight(),
    ...
    replaceSelection: function(code, updateInput, collapse) {
      ...
      this.setSelection(from, to, updateInput);
    },
    ...
    // TODO these don't play nice with highlighting yet
    ...
    startWorker: function(time) {
      ...
      this.highlightTimeout = setTimeout(function(){self.highlightWorker();}, time);
    }
  };
