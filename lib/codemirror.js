...
window.CodeMirror = (function() {
  ...
  function posFromMouse(cm, e, liberal) {
    ...
    if (ipad) {
      ...
      scrollX = window.pageXOffset || (document.documentElement || document.body).scrollLeft;
    }
    return coordsChar(cm, x - space.left - scrollX, y - space.top - scrollY);
  }
  ...
  function removeReadOnlyRanges(doc, from, to) {
    ...
    for (var i = 0; i < markers.length; ++i) {
      ...
      if (mk.inclusiveLeft) {
        m.from = {line: m.from.line, ch: m.from.ch - 1};
        if (m.from.ch < 0 && m.from.line > doc.first)
          m.from = {line: m.from.line - 1, ch: getLine(doc, m.from.line - 1).text.length};
      }
      if (mk.inclusiveRight) {
        m.to = {line: m.to.line, ch: m.to.ch + 1};
        if (m.to.line < doc.first + doc.size - 1 && m.to.ch > getLine(doc, m.to.line).text.length)
          m.to = {line: m.to.line + 1, ch: 0};
      }
  ...
  function runMode(cm, text, mode, state, f) {
    ...
    while (!stream.eol()) {
      ...
      stream.start = stream.pos;
      // Force splitting of tokens into single-character spans for iPad due to
      // not being able to reliably measure characters one-by-one. (On longer
      // lines, the cursor will not be in the correct location without this
      // hack.)
      if (!flattenSpans || curStyle != style || ipad) {
        if (ipad) {
        ...
        for (var i = 0; i < tokens.length; i++) {
          ...
          curText = tokens[i]; curStyle = style;
        }
