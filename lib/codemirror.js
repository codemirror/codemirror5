...
//    te.style.left = te.style.top = "-100000px";
    ...
    if (options.lineNumbers) {
      ...
      this.lineNumbers.className = "CodeMirror-line-numbers";
    }
  ...
  CodeMirror.prototype = {
    ...
    replaceLines: function(from, to, newText) {
      ...
      var ln = this.lineNumbers, l = lines.length;
    ...
    displaySelection: function() {
      ...
      var head = sel.inverted ? sel.from : sel.to, div = this.lines[head.line].div;
      // TODO properly take offsetParent into account
      var curtop = div.offsetTop, curleft = div.offsetLeft + head.ch * this.charWidth();

      var screen = this.code.clientHeight, screentop = this.code.scrollTop;
      if (curtop < screentop)
        this.code.scrollTop = Math.max(0, curtop - 10);
      else if ((curtop += this.lineHeight()) > screentop + screen)
        this.code.scrollTop = (curtop + 10) - screen;

      var screenw = div.offsetWidth, screenleft = this.code.scrollLeft;
      if (curleft < screenleft)
        this.code.scrollLeft = Math.max(0, curleft - 10);
      else if (curleft > screenw + screenleft)
        this.code.scrollLeft = (curleft + 10) - screenw;
    },
    ...
    mouseEventPos: function(e) {
      var target = e.target(), ln = this.lines.length;
      if (/span/i.test(target.nodeName)) target = target.parentNode;
      if (target == this.code) {
        var lastLine = this.lines[ln-1].div;
        var y = e.pageY() - eltOffset(lastLine).top;
        if (y > lastLine.offsetHeight) return {line: ln-1, ch: 0};
      }
      else {
        // TODO this is linear complexity, might be bad for huge docs
        for (var i = 0; i < ln; i++) {
          if (this.lines[i].div === target)
            return {line: i, ch: Math.round((e.pageX() - eltOffset(target).left) / this.charWidth())};
        }
      }
    },
    ...
    findCursor: function() {
      ...
      if (posEq(sel.from, sel.to))
        return this.lines[sel.from.line].div.getElementsByClassName("CodeMirror-cursor")[0];
    },
    restartBlink: function() {
      clearInterval(this.blinker);
      var on = true, self = this;
      this.blinker = setInterval(function() {
        var cur = self.findCursor();
        if (cur)
