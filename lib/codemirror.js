...
//    inputDiv.style.width = "30em"; inputDiv.style.height = "5em"; input.rows = 3;
    ...
    connect(div, "scroll", function() {updateDisplay(); if (options.onScroll) options.onScroll(instance);});
    ...
    function fastPoll(keyId) {
      ...
      function p() {
        ...
        else if (misses++ < 4) {poll.set(100, p);}
        else slowPoll();
        endOperation();
      }
      poll.set(20, p);
    ...
    function scrollIntoView(x1, y1, x2, y2) {
      ...
      var screen = div.clientHeight, screentop = div.scrollTop, scrolled = false, result = true;
      if (y1 < screentop) {div.scrollTop = Math.max(0, y1 - 10); scrolled = true;}
      else if (y2 > screentop + screen) {div.scrollTop = y2 + 10 - screen; scrolled = true;}

      var screenw = div.clientWidth, screenleft = div.scrollLeft;
      if (x1 < screenleft) {div.scrollLeft = Math.max(0, x1 - 10); scrolled = true;}
