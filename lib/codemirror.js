...
window.CodeMirror = (function() {
  ...
  Doc.prototype = createObj(BranchChunk.prototype, {
    ...
    findMarksAt: function(pos) {
      return this.findMarksInRange(pos, Pos(pos.line, pos.ch + 1));
    },
    ...
    findMarksInRange: function(from, to) {
      ...
      to = clipPos(this, to);
      var markers = [];
      for(var lineIndex = from.line; lineIndex <= to.line; ++lineIndex) {
        var isFirstLine = (lineIndex == from.line), isLastLine = (lineIndex == to.line);
        var line = getLine(this, lineIndex), spans= line.markedSpans;
        if (spans) for (var i = 0; i < spans.length; ++i) {
          var span = spans[i];
          if ((!isLastLine || span.from == null || span.from < to.ch) &&
              (!isFirstLine || span.to == null || span.to >= from.ch) || 
              (span.to <= to.ch && span.from <= from.ch)){
                var marker = span.marker.parent || span.marker;
                if(isFirstLine || marker.lines == null || marker.lines[0] == line)
                  markers.push(marker);
          }
        }
      }
