...
window.CodeMirror = (function() {
  ...
  function visibleLines(display, doc, scrollTop) {
    ...
    var toHeight = Math.ceil(top + display.scroller.clientHeight);
  ...
  function lineContent(cm, line, anchorAt) {
    ...
    function addSpan(text, style) {
      ...
      if (!specials.test(text)) {
        ...
        while (true) {
          ...
          pos += skipped + 1;
          if (m[0] == "\t") {
            var tabWidth = cm.options.tabSize - col % cm.options.tabSize;
    ...
    var span = addSpan;
    if (anchorAt != null) span = function(text, style) {
      var l = text.length;
      if (anchorAt >= pos && anchorAt < pos + l) {
        var cut = anchorAt - pos;
        if (anchorAt > pos) {
        ...
        if (cut + 1 == l) {
          pre.anchor = addSpan(text.slice(cut), style || "");
          anchorAt--;
        } else {
          var end = cut + 1;
          while (isExtendingChar.test(text.charAt(end))) ++end;
          pre.anchor = addSpan(text.slice(cut, end), style || "");
    ...
    insertLineContent(line, span);
    return pre;
  }

  // Outputs a number of spans to make up a line, taking highlighting
  // and marked text into account.
  function insertLineContent(line, span) {
    if (!line.text)
      span("\u00a0");
    else if (!line.markedSpans)
      for (var st = line.styles, i = 0; i < st.length; i+=2)
        span(st[i], styleToClass(st[i+1]));
    else // Complicated case
      insertLineContentMarked(line, span);
  }

  function insertLineContentMarked(line, span) {
    var st = line.styles, marked = line.markedSpans;
