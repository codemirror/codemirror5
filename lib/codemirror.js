var CodeMirror = (function() {
  ...
  function Timer() {this.id = null;}
  Timer.prototype = {set: function(ms, f) {clearTimeout(this.id); setTimeout(f, ms);}};
  ...
  Line.prototype = {
    ...
    highlight: function(parser, state) {
      ...
      while (!stream.done()) {
        ...
        this.styles.push(this.text.slice(start, stream.pos));
        this.styles.push(style);
      }
      this.updateDOM();
    },
    updateDOM: function() {
      var html = [], st = this.styles, pos = 0, node = 0;
      var sfrom = this.selFrom, sto = this.selTo, sel = sfrom == null ? 2 : 0, curAt;
      function addPiece(text, style) {
        var len = text.length, cut;
        if (sel === 0) {
          var off = sfrom - pos;
          if (off === 0) {
            if (sfrom == sto) {sel = 2; curAt = node;}
            else sel = 1;
          }
      ...
      if (sel === 0 && sfrom === sto) curAt = node;
      else if (sel === 1 && sto == null) html.push("<span class=\"CodeMirror-selected\"> </span>");
      this.div.innerHTML = html.join("");
      if (curAt != null) this.div.insertBefore(this.parent.cursor, this.div.childNodes[curAt]);
    }
  };
  ...
  CodeMirror.prototype = {
    ...
    onDblClick: function(e) {
      this.selectWordAt(this.clipPos(this.mouseEventPos(e)));
      e.stop();
    },
    onDrop: function(e) {
