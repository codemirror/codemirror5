...
window.CodeMirror = (function() {
  ...
  CodeMirror.defaults = {
    ...
    blkCommentStart: null, blkCommentContinuation: null
  ...
  var commands = CodeMirror.commands = {
    ...
    newlineAndIndent: function(cm) {
      ...
      // insert '*' blkCommentContinuation inside block comment.
      if (token.className == "comment") {
        var commentStart = cm.getOption('blkCommentStart');
        var commentContinuation = cm.getOption('blkCommentContinuation');
        if (commentStart && token.string.indexOf(commentStart) == 0) {
          cm.replaceSelection(" " + commentContinuation + " ", "end");
        } else if (token.state.tokenize != null && commentContinuation && token.string.indexOf(commentContinuation) == 0) {
          cm.replaceSelection(commentContinuation + " ", "end");
        }
      }
    },
