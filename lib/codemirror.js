var CodeMirror = (function() {
  ...
  var tabSize = 8; // TODO see if this is valid on different browsers
  ...
  Line.prototype = {
    ...
    getHTML: function(sfrom, sto) {
      ...
      function span(text, style) {
        ...
        else html.push(prepareCode(text));
      }
...
//    inputDiv.style.width = "30em"; inputDiv.style.height = "5em"; input.rows = 3;
    ...
    // TODO this is so clever that I don't understand it anymore
    function updateDisplay(changes) {
      ...
      else if (showingTo + added > to)
        updates.push({from: to, to: to, size: showingTo + added - to, at: at});
    ...
    // TODO heuristic indentation when no parser indentation or no state available?
    function indentLine(n) {
      if (!parser.indent) return;
      var state = getStateBefore(n);
      if (!state) return;
      var line = lines[n], curSpace = line.indentation(), curSpaceString = line.text.match(/^\s*/)[0];
      var indentation = parser.indent(state, line.text.slice(curSpace)), diff = indentation - curSpace;
      if (!diff) {
      ...
      var from = copyPos(sel.from), to = copyPos(sel.to), chDiff = indentString.length - curSpaceString.length;
      if (from.line == n) from.ch = Math.max(indentString.length, from.ch + chDiff);
      if (to.line == n) to.ch = Math.max(indentString.length, to.ch + chDiff);
      updateLines({line: n, ch: 0}, {line: n, ch: curSpaceString.length}, indentString, from, to);
    }
    ...
    function replaceRange1(code, from, to, computeSel) {
      var nl = code.indexOf("\n"), nls = 0;
      for (i = nl; i > -1; i = code.indexOf("\n", i + 1)) {++nls; nl = i;}
      var endch = nl == -1 ? code.length : code.length - nl - 1;
      var diff = nls - (to.line - from.line);
    ...
    function setParser(pname, config) {
      options.parser = pname; options.parserConfig = config;
    ...
    function highlightWorker(start) {
      ...
      while (work.length) {
        ...
        var minindent = lines[task].indentation(), minline = task;
        for (var e = Math.max(1, task - 50); task >= e; --task) {
          var line = lines[task-1], state = line.stateAfter, indent = line.indentation();
          if (state) break;
          if (indent < minindent) {minindent = indent; minline = task;}
        }
    ...
    var instance = {
      ...
      setOption: function(option, value) {
        ...
        else if (option === "indentUnit") {options.indentUnit = indentUnit = value; setParser(options.parser, options.parserConfig);}
        else if (/^(?:indentWithTabs|readOnly|autoMatchBrackets|undoDepth)$/.test(option)) options[option] = value;
  ...
  StringStream.prototype = {
    ...
    eatWhile: function(match) {while (this.eat(match)){}},
