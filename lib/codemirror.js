...
window.CodeMirror = (function() {
  ...
  function lineLength(doc, line) {
    ...
    while (merged = collapsedSpanAt(cur, "start")) {
      var found = merged.find();
      cur = getLine(doc, found.from.line);
      len += found.from.ch - found.to.ch;
    }
    cur = line;
    while (merged = collapsedSpanAt(cur, "end")) {
      var found = merged.find();
  ...
  function updateDisplayInner(cm, changes, scrollTop) {
    ...
    if (changes !== true && sawCollapsedSpans)
      for (var i = 0; i < changes.length; ++i) {
        ...
        while (merged = collapsedSpanAt(getLine(doc, ch.from), "start")) {
  ...
  function updateSelectionRange(cm) {
    ...
    function drawForLine(line, fromArg, toArg, retTop) {
      ...
      iterateBidiSections(getOrder(lineObj), fromArg || 0, toArg == null ? lineLen : toArg, function(from, to, dir) {
        ...
        if (fromArg == null && from == 0) left = paddingLeft(display);
    ...
    if (sel.from.line == sel.to.line) {
      ...
      if (singleLine) {
        ...
        if (!sel.to.ch)
          ...
          middleBot = drawForLine(sel.to.line, collapsedSpanAt(toObj, "start") ? null : 0, sel.to.ch, true);
  ...
  function measureLine(cm, line, ch) {
    ...
    for (var i = 0; i < cache.length; ++i) {
      var memo = cache[i];
      if (memo.ch == ch && memo.text == line.text && memo.markedSpans == line.markedSpans &&
    ...
    var memo = {ch: ch, text: line.text, width: display.scroller.clientWidth,
  ...
  function coordsChar(cm, x, y) {
    ...
    for (;;) {
      ...
      var merged = collapsedSpanAt(lineObj, "end");
  ...
  function coordsCharInner(cm, lineObj, lineNo, x, y) {
    ...
    for (;;) {
      if (bidi ? to == from || to == moveVisually(lineObj, from, 1) : to - from <= 1) {
        ...
        var collapsed = collapsedSpanAt(lineObj, ch);
        if (collapsed) ch = collapsed.find().from.ch;
  ...
  function endOperation(cm) {
    ...
    if (view.maxLineChanged && !cm.options.lineWrapping) {
      var width = measureLine(cm, view.maxLine, Infinity).left;
  ...
  function updateDocNoUndo(cm, from, to, lines, selUpdate) {
    ...
    if (!cm.options.lineWrapping) {
      for (var cur = firstLine, merged; merged = collapsedSpanAt(cur, "start");) {
    ...
    search: for (;;) {
      ...
      if (line.markedSpans) {
        for (var i = 0; i < line.markedSpans.length; ++i) {
          ...
          if (m.atomic && (sp.from == null || (m.inclusiveLeft ? sp.from <= curPos.ch : sp.from < curPos.ch))
  ...
  function addMarkedSpan(line, span) {
    line.markedSpans = line.markedSpans ?
      sortMarkedSpans(line.markedSpans.concat([span])) : [span];
    span.marker.lines.push(line);
  }
  function sortMarkedSpans(spans) {
    if (spans && spans.length > 1) spans.sort(function(a, b) { return a.from - b.from; });
    return spans;
  }
  ...
  function updateMarkedSpans(oldFirst, oldLast, startCh, endCh, newText) {
    ...
    var newMarkers = [newHL(newText[0], sortMarkedSpans(first))];
    if (!sameLine) {
      ...
      sortMarkedSpans(gapMarkers);
      for (var i = 0; i < gap; ++i)
        newMarkers.push(newHL(newText[i+1], gapMarkers));
      newMarkers.push(newHL(lst(newText), sortMarkedSpans(last)));
    }
    return newMarkers;
  }
  ...
  function collapsedSpanAt(line, ch) {
    ...
    if (!sps) return;
    if (ch == "end") ch = line.text.length + 1;
    else if (ch == "start") ch = -1;
    ...
    if (sps) for (var sp, i = 0; i < sps.length; ++i) {
      ...
      if (sp.from == null ||
          (sp.from == 0 && sp.marker.inclusiveLeft &&
           (sp.to == null || (sp.to == line.text.length && sp.marker.inclusiveRight))))
        return true;
    }
  }
  ...
  function makeLine(text, markedSpans, height) {
    ...
    if (markedSpans && collapsedSpanAt(line, "start")) line.height = 0;
    return line;
  }
  ...
  function updateLine(cm, line, text, markedSpans) {
    ...
    attachMarkedSpans(line, markedSpans);
    var hidden = collapsedSpanAt(line, "start");
  ...
  function lineContent(cm, line, anchorAt) {
    var merged;
    while (merged = collapsedSpanAt(line, "start")) {
      line = getLine(cm.view.doc, merged.find().from.line);
      anchorAt += line.text.length;
    }
      
    var builder = makeLineBuilder(cm, anchorAt);
    ...
    // Ensure the anchor is added, even when it is in a collapsed
    // range at the end of the line
    if (anchorAt != null && !builder.pre.anchor)
      builder.pre.anchor = builder.pre.appendChild(elt("span", " "));
    else if (!builder.pre.firstChild)
      builder.addSpan("\u00a0", null);

    return builder.pre;
  }

  function makeLineBuilder(cm, anchorAt) {
    var builder = {pre: elt("pre"), col: 0, pos: 0, isMeasure: anchorAt != null};
    var specials = /[\t\u0000-\u0019\u200b\u2028\u2029\uFEFF]/g;

    function addSpan(text, style) {
      if (!text) return;
      // Work around a bug where, in some compat modes, IE ignores leading spaces
      if (ie && text.charAt(0) == " " && !builder.pre.firstChild) text = "\u00a0" + text.slice(1);
      if (!specials.test(text)) {
        ...
        while (true) {
          ...
          pos += skipped + 1;
          if (m[0] == "\t") {
            var tabWidth = cm.options.tabSize - builder.col % cm.options.tabSize;
      ...
      if (style != null) return builder.pre.appendChild(elt("span", [content], style));
      else return builder.pre.appendChild(content);
    }

    builder.addSpan = addSpan;

    if (anchorAt != null) builder.addSpan = function(text, style) {
      var l = text.length;
      if (anchorAt < builder.pos + l && (anchorAt >= builder.pos || !builder.pre.anchor)) {
        var cut = anchorAt - builder.pos;
        if (cut > 0) {
          addSpan(text.slice(0, cut), style);
          // See comment at the definition of spanAffectsWrapping
          if (cm.options.lineWrapping && spanAffectsWrapping.test(text.slice(cut - 1, cut + 1)))
            builder.pre.appendChild(elt("wbr"));
        }
        if (cut + 1 == l) {
          builder.pre.anchor = addSpan(text.slice(cut), style || "");
          anchorAt--;
        } else {
          var end = cut + 1;
          if (cut < 0) {
            builder.pre.anchor = elt("img", null, null, "height: 5px; width: 0");
            var last = builder.pre.lastChild, widgetBefore = last && /\bCodeMirror-widget\b/.test(last.className);
            builder.pre.insertBefore(builder.pre.anchor, widgetBefore ? last : null);
            end = 0;
          } else {
            while (isExtendingChar.test(text.charAt(end))) ++end;
            builder.pre.anchor = addSpan(text.slice(cut, end), style || "");
            if (cm.options.lineWrapping && spanAffectsWrapping.test(text.slice(cut, end + 1)))
              builder.pre.appendChild(elt("wbr"));
          }
          addSpan(text.slice(end), style);
        }
      } else {
        addSpan(text, style);
      }
  ...
  function insertLineContent(line, builder) {
    if (!line.markedSpans)
      for (var st = line.styles, i = 0; i < st.length; i+=2)
        builder.addSpan(st[i], styleToClass(st[i+1]));
    else // Complicated case
      return insertLineContentMarked(line, builder);
  }

  function insertLineContentMarked(line, builder) {
    var st = line.styles, spans = line.markedSpans;
    var allText = line.text, len = allText.length;
    var pos = 0, i = 0, text = "", style, sp;
    var nextChange = spans[0].from || 0, activeSpans = [], markpos = 0;

    for (;;) {
      if (nextChange == pos) { // Update current marker set
        while (markpos < spans.length &&
               ((sp = spans[markpos]).from == pos || sp.from == null)) {
          // If the rest of the line is collapsed, bail out
          if (sp.marker.collapsed) {
            if (sp.marker.replacedWith && sp.from != null)
              builder.pre.appendChild(builder.isMeasure ? sp.marker.replacedWith.cloneNode(true)
                                                        : sp.marker.replacedWith);
            if (sp.to == null) {
              builder.pos += len - pos;
              return sp.marker.find();
            }
          }
          if (sp.marker.className || sp.marker.collapsed) activeSpans.push(sp);
          ++markpos;
        }
        nextChange = markpos < spans.length ? spans[markpos].from : Infinity;
        for (var j = 0; j < activeSpans.length; ++j) {
          var to = activeSpans[j].to;
          if (to == null) to = Infinity;
          if (to == pos) activeSpans.splice(j--, 1);
      ...
      while (true) {
        if (text) {
          ...
          var appliedStyle = style, collapsed = false;
          for (var j = 0; j < activeSpans.length; ++j) {
            var sp = activeSpans[j];
            if (sp.marker.collapsed) { collapsed = true; break; }
            appliedStyle = (appliedStyle ? appliedStyle + " " : "") + sp.marker.className;
            if (sp.marker.endStyle && sp.to === Math.min(end, upto)) appliedStyle += " " + sp.marker.endStyle;
            if (sp.marker.startStyle && sp.from === pos) appliedStyle += " " + sp.marker.startStyle;
          }
          if (collapsed)
            builder.pos += Math.min(end, upto) - pos;
          else
            builder.addSpan(end > upto ? text.slice(0, upto - pos) : text, appliedStyle);
  ...
  function heightAtLine(cm, lineObj) {
    var merged;
    while (merged = collapsedSpanAt(lineObj, "start"))
  ...
  function lineStart(cm, lineNo) {
    var merged, line;
    while (merged = collapsedSpanAt(line = getLine(cm.view.doc, lineNo), "start"))
  ...
  function lineEnd(cm, lineNo) {
    var merged, line;
    while (merged = collapsedSpanAt(line = getLine(cm.view.doc, lineNo), "end"))
      lineNo = merged.find().to.line;
    var order = getOrder(line);
