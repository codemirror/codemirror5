  ...
  function findPosH(doc, pos, dir, unit, visually) {
    ...
    function getHandler (str) {
      for ( var handler in handlers ) {
        if ( handlers[handler].test(str) ) {
          handlers[handler].count++;
          return handlers[handler];
        }
      }

      return handlers.delimeter;
    }
    ...
    if (unit == "char") moveOnce();
      ...
      for (;;) {
        ...
        // Skipping words forward works very nicely.  Gotta make some adjustments for
        // backward navigation...
        ...
        var _char = lineObj.text.charAt(ch);
        var handler = getHandler(_char);

        // If we have a white, we will simply go to the next character...
        if ( handler.type === "space" ) {
        }
        else if (handler.type === "empty" ) {
          ...
          if ( handlers.delimeter.count || handlers.character.count ) {
              break;
          }
        ...
        else if ( handler.type === "delimeter" ) {
          // We only exit if we have seen any characters...
          if ( handlers.character.count || handlers.space.count ) {
            if (dir < 0) {
              dir = 1;
              moveOnce();
            }
          ...
          if ( handlers.delimeter.count || handlers.space.count ) {
            if (dir < 0) {
              dir = 1;
              moveOnce();
            }
