...
window.CodeMirror = (function() {
  ...
  function maybeUpdateLineNumberWidth(cm) {
    ...
    if (last.length != display.lineNumChars) {
      ...
      display.lineNumWidth = Math.max(innerW, display.lineGutter.offsetWidth - padding);
      display.lineNumChars = display.lineNumWidth ? last.length : -1;
      display.lineGutter.style.width = (display.lineNumWidth + padding) + "px";
      return true;
    }
    return false;
  }
  ...
  function patchDisplay(cm, from, to, intact, updateNumbersFrom) {
    ...
    var fixedPos = compensateForHScroll(display);
        ...
        if (line.hidden) var lineElement = elt("div");
          ...
          if (lineNumbers || markers || line.bgClassName || (line.widgets && line.widgets.length)) {
            ...
            if (lineNumbers || markers) {
              var gutterWrap = wrap.appendChild(elt("div", null, null,
                                                    "position: absolute; left: " + fixedPos + "px"));
              wrap.alignable = [gutterWrap];
              if (lineNumbers)
                gutterWrap.appendChild(elt("div", lineNumberFor(cm.options, j),
                                           "CodeMirror-linenumber CodeMirror-gutter-elt",
