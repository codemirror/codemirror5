...
window.CodeMirror = (function() {
  ...
  function updateDisplayInner(cm, changes, viewPort) {
    ...
    var lastHeight = display.lineDiv.offsetTop;
    for (var node = display.lineDiv.firstChild; node; node = node.nextSibling) {
      var end = node.offsetHeight + node.offsetTop;
      var height = end - lastHeight, diff = node.lineObj.height - height;
  ...
  function patchDisplay(cm, from, to, intact, updateNumbersFrom) {
    ...
    cm.view.doc.iter(from, to, function(line) {
      ...
      if (lineIsHidden(line)) {
        ...
        if (lineNumbers && updateNumbersFrom <= lineNo)
          setTextContent(cur.firstChild.firstChild, lineNumberFor(cm.options, lineNo));
        cur = cur.nextSibling;
      } else {
  ...
  function updateDocInner(cm, from, to, newText, selUpdate, origin) {
    ...
    return updateDocNoUndo(cm, from, to, lines, selUpdate, origin);
  }

  function unredoHelper(cm, from, to, dir, origin) {
    var doc = cm.view.doc, hist = cm.view.history;
    if (!from.length) return;
    var set = from.pop(), anti = {events: [], selFrom: cm.view.sel.from, selTo: cm.view.sel.to};
    for (var i = set.events.length - 1; i >= 0; i -= 1) {
      ...
      var selPos = i == set.events.length - 1 ? {from: set.selFrom, to: set.selTo} : null;
      updateDocNoUndo(cm, {line: change.start, ch: 0}, {line: end - 1, ch: getLine(doc, end-1).text.length},
                      change.old, selPos, origin);
    }
    to.push(anti);
  }
  ...
  CodeMirror.prototype = {
    ...
    undo: operation(null, function() {
      ...
      unredoHelper(this, hist.done, hist.undone, -1, "undo");
    }),
    redo: operation(null, function() {
      var hist = this.view.history;
      unredoHelper(this, hist.undone, hist.done, 1, "redo");
    }),
    ...
    getHistory: function() {
      ...
      function cp(arr) {
        for (var i = 0, nw = [], nwelt; i < arr.length; ++i) {
          nw.push({events: nwelt = [], selFrom: arr[i].selFrom, selTo: arr[i].selTo});
          for (var j = 0, elt = arr[i].events; j < elt.length; ++j) {
            var old = [], cur = elt[j];
    ...
    refresh: function() {
      this.display.measureLineCache.length = 0;
  ...
  function addChange(cm, start, added, old, origin) {
    ...
    if (cur &&
      ...
      cur = {events: [{start: start, added: added, old: old}],
             selFrom: cm.view.sel.from, selTo: cm.view.sel.to};
