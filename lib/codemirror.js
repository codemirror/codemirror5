var CodeMirror = (function() {
  ...
  Line.prototype = {
    ...
    highlight: function(parser, state) {
      ...
      st.length = 0;
      while (!stream.done()) {
        var style = parser.token(stream, state, stream.start == 0);
    ...
    indentation: function() {
      var space = this.text.match(/^\s*/)[0], n = 0;
      for (var i = 0, e = space.length; i < e; ++i) {
        var ch = space.charAt(i);
        if (ch == "\t") n += 8 - (i % 8);
        else ++n;
      }
      return n;
...
//    inputDiv.style.width = "30em"; inputDiv.style.height = "5em"; input.rows = 3;
    ...
    function indentLine(n) {
      ...
      if (!state) return;
      var line = lines[n], curSpace = line.indentation();
  ...
  StringStream.prototype = {
    ...
    eatSpace: function() { // TODO tab counting
      var start = this.pos;
