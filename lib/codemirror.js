  ...
  function CodeMirror(place, givenOptions) {
    ...
    function updateSelection() {
      ...
      if (collapsed) {
        ...
        function add(left, top, width, height) {
          html += '<div class="CodeMirror-selected" style="position: absolute; left: ' + left +
            'px; top: ' + top + 'px; width: ' + width + 'px; height: ' + height + 'px"></div>';
        }
        if (sel.from.ch && fromPos.y >= 0) {
          var width = (sameLine ? toPos.x : lineSpace.clientWidth) - fromPos.x;
          add(fromPos.x, fromPos.y, width, th);
        }
        var middleStart = Math.max(0, fromPos.y + (sel.from.ch ? th : 0));
        var middleHeight = Math.min(toPos.y, lineSpace.clientHeight) - middleStart;
        if (middleHeight > 0.2 * th)
          add(0, middleStart, lineSpace.clientWidth, middleHeight);
        if ((!sameLine || !sel.from.ch) && toPos.y < lineSpace.clientHeight - .5 * th)
          add(0, toPos.y, toPos.x, th);
