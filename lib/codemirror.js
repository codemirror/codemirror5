...
var CodeMirror = (function ()
{
  ...
  function CodeMirror(place, givenOptions)
  {
    ...
    {
      var curAbsPos = 0;
      for (var i = 0; i < relPos.line && i < instance.lineCount(); i++)
      { // Count the \n char which is not retrieved
        curAbsPos += instance.getLine(i).length + 1;
      }
      curAbsPos += relPos.ch;
      return curAbsPos;
    }
    ...
    function absToRel(absPos)
    {
      ...
      for (i = 0; i < instance.lineCount(); i++)
      { // Count the \n char which is not retrieved
        absPos -= instance.getLine(i).length + 1;
        if (absPos <= 0)
        {
          relPos.line = i;
          relPos.ch = absPos + instance.getLine(i).length + 1;
          return relPos;
        }
      }

      // The absPos is out of the upper bound, return the end of the doc
      var lastLn = instance.lineCount() - 1;
      var lastCh = instance.getLine(lastLn).length - 1;
      return { line: lastLn, ch: lastCh };
    }

    // Adds extra \n chars to the specified text as needed and returns 
    // the wrapped text
    function getWrappedText(text, charsPerLine, wordWrapChars)
    {
    ...
    var instance = {
      // =========== Extended by A.K. =============
      // Returns a { from, to } object which describes the current selection
      // range.
      ...
      indentRange: function (isIncreaseIndent, from, to)
      {
        this.operation(function ()
        {
          for (var i = from.line, e = to.line; i <= e; ++i)
          {
            indentLine(i, isIncreaseIndent ? "add" : "subtract");
          }
        });
      },
      
      // Comment/uncomment the specified range
      commentRange: function (isComment, from, to)
      {
        var curMode = mode.getModeAtPos(this.getValue(), relToAbs(this.getCursor(sel.inverted)));
        if (isComment)
        { // Comment range
      ...
      wordWrapRange: function (from, to)
      {
        var textToWrap = this.getRange(from, to);
        var tabReplacement = "";
        for (var i = 0; i < options.indentUnit; i++)
        {
          tabReplacement += " ";
        }
        ...
        to = absToRel(relToAbs(from) + textToWrap.length);

        // Get the needed width (in chars)
        var charWidth = 7;
        if (this.getLine(0).length > 1)
        { // Measure the actual char width
          charWidth = (this.charCoords({ line: 0, ch: 1 }).x - this.charCoords({ line: 0, ch: 0 }).x)
        }

        var charsPerLine = -2
          + Math.round((this.getWrapperElement().clientWidth
            - (gutter != null ? gutter.clientWidth : 0))
            / charWidth);

        // Make the wrap
        var wrappedText = getWrappedText(textToWrap, charsPerLine, [",", "\\s"].concat(mode.wordWrapChars));
        this.operation(function ()
        {
          instance.replaceRange(wrappedText, from, to);
        });
      },
      
      // Applies automatic mode-aware indentation to the specified range
      autoIndentRange: function (from, to)
      {
        this.operation(function ()
        {
      ...
      {
        var absStart = relToAbs(from);
        var absEnd = relToAbs(to);
        // Insert additional line breaks where necessary according to the
        // mode's syntax
        var res = mode.autoFormatLineBreaks(this.getValue(), absStart, absEnd);

        // Replace and auto-indent the range
        this.operation(function ()
        {
          instance.replaceRange(res, from, to);
          var startLine = absToRel(absStart).line;
          var endLine = absToRel(absStart + res.length).line;
    ...
    function endOperation()
    {
      ...
      // ================= Changed by A.K. - fix from https://github.com/marijnh/CodeMirror2/commit/65f257ba6385638164986aa1a8a445ab22652793 applied ======
