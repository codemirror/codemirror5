...
window.CodeMirror = (function() {
  ...
  function CodeMirror(place, options) {
    if (!this || this == window) return new CodeMirror(place, options);
    ...
    function extend(e) {
      ...
      if (cur && !posEq(cur, last)) {
        ...
        if (cur.line >= visible.to || cur.line < visible.from)
          going = setTimeout(operation(cm, function(){extend(e);}), 150);
      }
    }
  ...
  function updateDocNoUndo(cm, from, to, lines, selFrom, selTo) {
    ...
    if (hasHandler(cm, "change")) {
      ...
      if (cm.curOp.textChanged) {
        for (var cur = cm.operaton.textChanged; cur.next; cur = cur.next) {}
        cur.next = changeObj;
  ...
  CodeMirror.prototype = {
    ...
    deleteH: operation(null, function(dir, unit) {
      var sel = this.view.sel;
      if (!posEq(sel.from, sel.to)) replaceRange(cm, "", sel.from, sel.to);
    ...
    getScrollInfo: function() {
      return {left: this.display.scroller.scrollLeft, top: this.display.scroller.scrollTop,
              height: this.display.scroller.scrollHeight, width: this.display.scroller.scrollWidth};
    },
