...
window.CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    function lineNumberLeftPos() {
      return scroller.getBoundingClientRect().left - sizer.getBoundingClientRect().left + lineGutter.offsetLeft - gutters.offsetWidth;
    }
    ...
    function updateDisplay(changes, suppressCallback, scrollTop) {
      ...
      if (changes && changes !== true && maybeUpdateLineNumberWidth())
        changes = true;
      mover.style.marginLeft = scrollbarH.style.left = gutters.offsetWidth + "px";
    ...
    function moveV(dir, unit) {
      var dist = 0, pos = cursorCoords(sel.inverted ? sel.from : sel.to, "div");
      var x = pos.left, y;
    ...
    function cursorCoords(pos, context, lineObj) {
      ...
      for (var i = 0; i < order.length; ++i) {
        ...
        if (left == ch) {
          // Opera return bogus offsets and widths for edges where the
          // direction flips, but only for the side with the lower
          // level.
          if (opera && i && part.level < (nb = order[i-1]).level) here = get(nb.level % 2 ? nb.from : nb.to - 1, true);
          else here = get(rtl ? ch - 1 : ch);
          if (rtl == linedir) main = here; else other = here;
        } else if (right == ch) {
          if (opera && i < order.length - 1 && part.level < (nb = order[i+1]).level) here = get(nb.level % 2 ? nb.to - 1 : nb.from);
    ...
    function endOperation() {
      ...
      if (maxLineChanged && !options.lineWrapping) {
        ...
        if (!ie_lt8)
          sizer.style.minWidth = (width + scrollerCutOff) + "px";
