...
window.CodeMirror = (function() {
  ...
  // provide a CSS-transform-agnostic altenative to gBGR
  function fakeGetBoundingClientRect(e){
   return {left: e.offsetLeft, top: 0, right: e.offsetLeft+e.offsetWidth, bottom: e.offsetHeight};
  }
  ...
  function updateDisplayInner(cm, changes, viewPort) {
    ...
    for (var node = display.lineDiv.firstChild, height; node; node = node.nextSibling) if (node.lineObj) {
      if (ie_lt8) {
        ...
        var box = fakeGetBoundingClientRect(node);
  ...
  function measureLineInner(cm, line) {
   ...
   var outer = fakeGetBoundingClientRect(display.lineDiv);
    ...
    for (var i = 0, cur; i < measure.length; ++i) if (cur = measure[i]) {
     var size = fakeGetBoundingClientRect(cur);
