var CodeMirror = (function() {
  ...
  function splitSpan(node, at) {
    var text = node.firstChild.nodeValue;
    node.firstChild.nodeValue = text.slice(0, at);
    var sp = textSpan(text.slice(at));
  ...
  CssClass.prototype = {
    add: function(node) {
      ...
      if (!this.find.test(cls)) node.className = this.extra + cls;
    },
    rem: function(node) {
  ...
  var selCls = new CssClass("CodeMirror-selected");

  function Line(div, parent) {
    this.div = div; this.parent = parent;
  }
  Line.prototype = {
    ...
    // TODO extra selection space for selected newlines
    clearSelection: function() {
      if (this.selFrom == null) return;
      for (var n = this.div.firstChild; n; n = n.nextSibling)
        selCls.rem(n);
      this.selFrom = this.selTo = null;
      this.joinSpans();
    },
    setSelection: function(from, to) {
      var pos = 0, inside = 0, next;
      for (var n = this.div.firstChild; n; n = n.nextSibling) {
        var len = n.firstChild.nodeValue.length;
        if (inside === 0) {
          var off = from - pos;
          if (off === 0) {
            if (from == to) {
              inside = 2;
              n.parentNode.insertBefore(this.parent.cursor, n);
            }
            else inside = 1;
          }
          else if (off < len) {splitSpan(n, off); len = off;}
        }
        if (inside === 1 && to != null) {
          var off = to - pos;
          if (off === 0) inside = 2;
        ...
        if (inside === 1) selCls.add(n);
        else selCls.rem(n);
        pos += len;
      }
      if (from === to && from === pos) this.div.appendChild(this.parent.cursor);
    ...
    joinSpans: function() {
      for (var n = this.div.firstChild; n; n = n.nextSibling) {
        ...
        if (nx && nx.className == n.className) {
          n.parentNode.removeChild(nx);
          n.firstChild.nodeValue += nx.firstChild.nodeValue;
        }
      }
    },
    highlight: function(parser, state) {
      var stream = new StringStream(this.text), html = [];
      while (!stream.done()) {
        var start = stream.pos, style = parser.token(stream, state, start == 0);
        var str = this.text.slice(start, stream.pos);
  ...
  CodeMirror.prototype = {
    ...
    onMouseDown: function(e) {
      ...
      if (e.pageX() - corner.left > this.code.clientWidth ||
          e.pageY() - corner.top > this.code.clientHeight) return;
    ...
    mouseEventPos: function(e) {
      // TODO deal with line-wrapping (elementFromPoint, binary
      // subdividing search in case of line-spanning spans)
