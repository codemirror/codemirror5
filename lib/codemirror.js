var CodeMirror = (function() {
  ...
  Line.prototype = {
    ...
    addMark: function(from, to, style) {
      ...
      this.marked.sort(function(a, b){return b.from - a.from;});
    },
    removeMark: function(from, to, style) {
      var mk = this.marked;
      if (to == null) to = this.text.length;
      if (!mk) return;
      for (var i = 0; i < mk.length; ++i) {
        var mark = mk[i];
        if (style && mark.style != style) continue;
        var mto = mark.to == null ? this.text.length : mark.to, del = false;
        if (mark.from >= from && mto <= to) del = true;
        else if (mark.from < from && mto > to) {
          mk.splice(++i, 0, {from: to, to: mark.to, style: mark.style});
          mark.to = from;
        }
        else if (mto > from && mark.from < from) mark.to = from;
        else if (mark.from < to && mto > to) mark.from = to;
        if (del || mark.from == mark.to) mk.splice(i--, 1);
      }
    },
...
//    inputDiv.style.width = "30em"; inputDiv.style.height = "5em"; input.rows = 3;
    ...
    function markHelper(from, to, func, className) {
      from = clipPos(from); to = clipPos(to);
      if (from.line == to.line) func.call(lines[from.line], from.ch, to.ch, className);
      else {
        func.call(lines[from.line], from.ch, null, className);
        for (var i = from.line + 1, e = to.line; i < e; ++i)
          func.call(lines[i], 0, null, className);
        func.call(lines[to.line], 0, to.ch, className);
      }
      changes.push({from: from.line, to: to.line + 1});
    }
    function markText(from, to, className) {markHelper(from, to, Line.prototype.addMark, className);}
    function unmarkText(from, to, className) {markHelper(from, to, Line.prototype.removeMark, className);}
    ...
    var instance = {
      ...
      unmarkText: operation(unmarkText),
