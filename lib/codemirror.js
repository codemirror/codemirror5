...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    function onCutCopy(e, isCut) {
      input.value = getSelection();
      leaveInputAlone = true;
      input.select();
      if (e.type == "cut" || isCut) operation(replaceSelection)("");
    ...
    function onKeyDown(e) {
      ...
      if (window.opera) {
        ...
        // Opera has no cut/copy events... we try to at least catch the keys
        if (!handled && (mac ? e.metaKey : e.ctrlKey) && (e.keyCode == 67 || e.keyCode == 88))
          onCutCopy(e, e.keyCode == 88);
      }
    }
    ...
    function resetInput() {
      prevInput = "";
      if (!window.opera && focused && !posEq(sel.from, sel.to)) {
        input.value = "\01";
        input.select();
      } else input.value = "";
    }
