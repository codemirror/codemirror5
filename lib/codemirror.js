      ...
      for (i = 0; i < inlines.length; i++) {
        ...
        if (!inline.exists()) {
          ...
          if (lineNum >= from && lineNum < to) {
            ...
            node.style.top = paddingTop() + localCoords({line: lineNum, ch: 0}, true).yBot + "px";
    ...
    function updateGutter() {
      ...
      doc.iter(showingFrom, Math.max(showingTo, showingFrom + 1), function(line) {
        if (line.hidden) {
          ...
          if (inline) {
            html.push(inline.createPlaceholder(targetDocument).outerHTML);
          }
        }
        ++i;
      });
...
/**/
var selEnd = (sel.inverted ? sel.from : sel.to);
console.log("moveV: start pos = line " + selEnd.line + ", ch " + selEnd.ch + ", start coords = (" + pos.x + ", " + pos.y + "), dist = " + dist);
console.log("target = line " + target.line + ", ch " + target.ch);
/**/
    ...
    // Inline widgets
    // TODO: what's the right way to handle wordwrapped lines?
    function addInlineWidget(pos, node, initialHeight) {
      ...
      var inlineWidget = new InlineWidget(nextInlineId++, bookmark, node, inlineStyleTag.sheet);
      for (i = 0; i < inlines.length; i++) {
        if (inlines[i].exists() && inlines[i].getLineNumber() > pos.line)
          break;
      }
    ...
    function getInlineWidgetForLine(line) {
      for (var i = 0; i < inlines.length; i++) {
        if (inlines[i].exists() && inlines[i].bookmark.line === line)
          return inlines[i];
      }
      return null;
    }
    
    function getInlineWidgetForId(id) {
      for (var i = 0; i < inlines.length; i++) {
        if (inlines[i].exists() && inlines[i].getId() == id)
          return inlines[i];
      }
      return null;
    }
  ...
  var inlineClassPrefix = "CM-inline", inlinePlaceholderAttr = "data-cm-inline";
  function InlineWidget(id, bookmark, node, styleSheet) {
  	this.id = id;
  	this.bookmark = bookmark;
  	this.node = node;
    this.height = 0;
    
    var newIndex = styleSheet.insertRule('.' + this.getInlineWidgetClass() + '{height: 0}');
    this.styleRule = styleSheet.cssRules.item(newIndex);
    this.styleSheet = styleSheet;
    
    node.setAttribute('class', inlineClassPrefix + ' ' + this.getInlineWidgetClass());
  }
  InlineWidget.prototype = {
    ...
    getLineNumber: function() {
  	...
  	setHeight: function(height) {
  	  this.height = height;
      this.styleRule.style.setProperty('height', this.height + 'px');
  	},
    ...
    hide: function() {
      ...
      placeholder.setAttribute(inlinePlaceholderAttr, this.getId());
      return placeholder;
  	},
