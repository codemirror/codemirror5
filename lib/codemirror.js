...
window.CodeMirror = (function() {
  ...
  function getStateBefore(cm, n) {
    var doc = cm.view.doc;
    var pos = findStartLine(cm, n), state = pos && getLine(doc, pos-1).stateAfter;
  ...
  function updateDoc(cm, from, to, newText, selFrom, selTo) {
    ...
    if (doc.history) {
      addChange(doc.history, from.line, newText.length, old);
  ...
  function indentLine(cm, n, how) {
    ...
    var line = getLine(doc, n), curSpace = countColumn(line.text, null, cm.options.tabSize);
    ...
    if (how == "prev") {
      if (n) indentation = countColumn(getLine(doc, n-1).text, null, cm.options.tabSize);
      else indentation = 0;
    }
  ...
  CodeMirror.prototype = {
    ...
    clearHistory: function() {this.view.doc.history = makeHistory();},
    ...
    setHistory: function(histData) {
      var hist = this.view.doc.history = makeHistory();
    ...
    getTokenAt: function(pos) {
      ...
      pos = clipPos(doc, pos);
      return getLine(doc, pos.line).getTokenAt(doc.mode, getStateBefore(this, pos.line),
    ...
    markText: operation(null, function(from, to, className, options) {
      ...
      addChange(doc.history, from.line, old.length, old, true);
      return marker;
    }),
    ...
    setBookmark: function(pos) {
      ...
      line.markedSpans = (line.markedSpans || []).concat([span]);
      marker.lines.push(line);
      addChange(doc.history, pos.line, 1, [old]);
      return marker;
    },
  ...
  TextMarker.prototype.clear = function() {
    ...
    if (min != null) {
      ...
      addChange(this.cm.view.doc.history, min, old.length, old, true);
    }
    this.lines.length = 0;
  ...
  function highlightLine(cm, line, state) {
    ...
    while (!stream.eol()) {
      ...
      if (stream.pos > 5000) {
        st[pos++] = line.text.slice(stream.pos); st[pos++] = null;
        break;
      }
    }
  }
