  ...
  Line.inheritMarks = function(text, orig) {
    ...
    if (mk) {
      for (var i = 0; i < mk.length; ++i) {
        if (mk[i].to == null)
          (ln.marked || (ln.marked = [])).push({from: null, to: null, style: mk[i].style});
      }
    }
    return ln;
  }
      ...
      if (mk) {
        for (var i = 0; i < mk.length; ++i) {
          ...
          if (mark.to > pos || mark.to == null)
            (taken.marked || (taken.marked = [])).push({
              from: mark.from < pos ? null : mark.from - pos + textBefore.length,
        ...
        outer: for (var i = 0; i < mk.length; ++i) {
          ...
          if (!mark.from) {
            for (var j = 0; j < mymk.length; ++j) {
              var mymark = mymk[j];
              if (mymark.to == mylen && mymark.style == mark.style) {
                mymark.to = mark.to + mylen;
                continue outer;
              }
            }
          }
