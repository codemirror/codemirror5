  ...
  function CodeMirror(place, givenOptions) {
    ...
    function onKeyDown(e) {
      ...
      if (!options.readOnly) {
        ...
        if (!anyMod && code == 8 && handleBackspace()) return e_preventDefault(e); // backspace 
    ...
    function handleBackspace() {
        if (options.tabMode == "shift") {
          if (posEq(sel.from, sel.to)
              && sel.from.ch != 0
              && lines[sel.from.line].indentation() == sel.from.ch) {
            indentSelected("subtract");
            return true;
          }
        }
        return false;
