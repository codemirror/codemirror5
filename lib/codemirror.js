...
//    inputDiv.style.width = "30em"; inputDiv.style.height = "5em"; input.rows = 3;
    ...
    function handleEnter() {
      ...
      if (options.enterMode != "flat")
        indentLine(sel.from.line, options.enterMode == "keep");
    }
    function handleTab() {
      for (var i = sel.from.line, e = sel.to.line; i <= e; ++i)
        indentLine(i);
    }
    function indentLine(n, usePrevious) {
      if (!parser.indent) usePrevious = true;
      if (!usePrevious) {
        var state = getStateBefore(n);
        if (!state) usePrevious = true;
      }

      var line = lines[n], curSpace = line.indentation(), curSpaceString = line.text.match(/^\s*/)[0];
      if (usePrevious) {
        if (n) var indentation = lines[n-1].indentation();
        else var indentation = 0;
      }
      else
        var indentation = parser.indent(state, line.text.slice(curSpace));
