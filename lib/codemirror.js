var CodeMirror = (function() {
  ...
  Line.prototype = {
    ...
    updateDOM: function() {
      ...
      if (pos === sfrom && sfrom === sto) html.push("<span class=\"CodeMirror-cursor\">\u200b</span>");
      else if (sel === 1 && sto == null) html.push("<span class=\"CodeMirror-selected\"> </span>");
...
//    input.style.left = input.style.top = "-100000px";
    ...
    var parser = parsers[options.parser || defaultParser];
    if (!parser) throw new Error("No parser found.");
    
    var lines = []; var work = [];
    var zero = {line: 0, ch: 0};
    var sel = {from: zero, to: zero, inverted: false};
    var prevSelection = {from: zero, to: zero};
    var shiftSelecting, reducedSelection, focused, updateInput;
    ...
    function setValue(code) {
      replaceLines(0, lines.length, code.split(/\r?\n/g));
      setCursor(0);
    }
    ...
    function onMouseDown(e) {
      ...
      var move = connect(window, "mousemove", operation(function(e) {
      ...
      var up = connect(window, "mouseup", operation(function(e) {
        var cur = mouseEventPos(e);
        if (cur) setSelection(start, cur);
        end();
      }), true);
      var leave = connect(window, "mouseout", operation(function(e) {
    ...
    function replaceLines(from, to, newText) {
    ...
    function readInput() {
      ...
      if (changed) shiftSelecting = reducedSelection = null;
      ...
      if (changed) {
        shiftSelecting = null;
        replaceLines(editing.from, editing.to, text.split(/\r?\n/g));
      }
    ...
    function displaySelection() {
      var pr = prevSelection;
      if (posEq(pr.from, sel.from) && posEq(pr.to, sel.to)) return;

      for (var i = pr.from.line, e = Math.min(lines.length, sel.from.line, pr.to.line + 1); i < e; i++)
        lines[i].setSelection(null);
      for (var i = Math.max(sel.to.line + 1, pr.from.line), e = Math.min(pr.to.line, lines.length); i <= e; i++)
        lines[i].setSelection(null);
    ...
    function setCursor(line, ch) {
      if (line < 0) {line = 0; ch = 0;}
      if (line > lines.length - 1) {line = lines.length - 1; ch = lines[line].text.length;}
      var pos = {line: line, ch: ch ? Math.min(lines[line].text.length, ch) : 0};
      setSelection(pos, pos);
    }
    ...
    function indentLine(n) {
      ...
      if (diff > 0) {
        ...
        replaceLines(n, n + 1, [space + text]);
      }
      else
        replaceLines(n, n + 1, [text.slice(-diff)]);
    ...
    function replaceSelection(code, collapse) {
      code = code.split(/\r?\n/g);
      code[0] = lines[sel.from.line].text.slice(0, sel.from.ch) + code[0];
      var endch = code[code.length-1].length;
      code[code.length-1] += lines[sel.to.line].text.slice(sel.to.ch);
      var from = sel.from, to = {line: sel.from.line + code.length - 1, ch: endch};
      replaceLines(sel.from.line, sel.to.line + 1, code);
    ...
    function mouseEventPos(e) {
      ...
      if (target == code) {
        ...
        var ch = Math.round((e.pageX() - eltOffset(target.firstChild).left) / charWidth());
        for (var i = 0; i < ln; i++) {
          var line = lines[i];
          if (line.div === target)
    ...
    function findCursor() {
      if (posEq(sel.from, sel.to))
        return lines[sel.from.line].div.getElementsByClassName("CodeMirror-cursor")[0];
    }
    ...
    function highlightWorker(start) {
      ...
      while (work.length) {
        var task = work.pop()
        if (lines[task].stateAfter) continue;
        if (task) {
          var state = lines[task-1].stateAfter;
          if (!state) continue;
          state = copyState(state);
        }
        else var state = parser.startState();
    ...
    function endOperation() {
      ...
      if (ps.from.line != sel.from.line || ps.to.line != sel.to.line || updateInput)
        prepareInput();
    }
