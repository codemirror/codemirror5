...
window.CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    var instance = {
      ...
      disconnect: function(type, f) {disconnect(this, type, f);},
    ...
    connect(gutters, "mousedown", operation(clickInGutter));
    connect(scroller, "dblclick", operation(onDoubleClick));
    connect(lineSpace, "selectstart", function(e) {
    ...
    connect(scrollbarH, "scroll", onScrollBarH);
    connect(scrollbarV, "scroll", onScrollBarV);
    connect(scrollbarH, "mousedown", function() {if (focused) setTimeout(focusInput, 0);});
    connect(scrollbarV, "mousedown", function() {if (focused) setTimeout(focusInput, 0);});
    connect(wrapper, "scroll", function() { wrapper.scrollTop = wrapper.scrollLeft = 0; });
    connect(window, "resize", function resizeHandler() {
      if (wrapper.parentNode) updateDisplay(true);
      else disconnect(window, "resize", resizeHandler);
    });
    ...
    function alignLineNumbers() {
      if (alignWidgets) {
        var margin = compensateForHScroll() + "px";
        for (var n = lineDiv.firstChild; n; n = n.nextSibling)
          for (var c = n.lastChild; c && /\bCodeMirror-linewidget-noinline\b/.test(c.className); c = c.prevSibling)
    ...
    function onMouseDown(e) {
      ...
      if (mouseEventInWidget(e) || clickInGutter(e)) return;
      ...
      if (options.dragDrop && dragAndDrop && !options.readOnly && !posEq(sel.from, sel.to) &&
        ...
        var dragEnd = operation(function(e2) {
          ...
          draggingText = false;
          disconnect(document, "mouseup", dragEnd);
          disconnect(scroller, "drop", dragEnd);
        ...
        connect(document, "mouseup", dragEnd);
        connect(scroller, "drop", dragEnd);
        return;
      }
      e_preventDefault(e);
      ...
      function done(e) {
        ...
        focusInput();
        updateInput = true;
        disconnect(document, "mousemove", move);
        disconnect(document, "mouseup", up);
      }
      ...
      connect(document, "mousemove", move);
      connect(document, "mouseup", up);
    }
            ...
            if (lineNumbers || markers || line.bgClassName || (line.widgets && line.widgets.length)) {
              ...
              if (line.widgets)
                for (var i = 0, ws = line.widgets; i < ws.length; ++i) {
                  ...
                  if (!widget.inline) {
                    node.className += " CodeMirror-linewidget-noinline";
                    node.style.marginLeft = compensateForHScroll() + "px";
                  }
                  inside.push(node);
                }
    ...
    function addLineWidget(handle, node, inline) {
      if (!inline) alignWidgets = true;
      var widget = {node: node, inline: inline};
    ...
    function cursorCoords(pos, context, lineObj) {
      ...
      for (var i = 0; i < order.length; ++i) {
        ...
        if (left == ch) {
          ...
          else here = get(rtl ? ch - 1 : ch);
          if (rtl == linedir) main = here; else other = here;
        } else if (right == ch) {
          if (i < order.length - 1 && part.level < (nb = order[i+1]).level) here = get(nb.level % 2 ? nb.to - 1 : nb.from);
    ...
    function onContextMenu(e) {
      ...
      if (gecko) {
        e_stop(e);
        connect(window, "mouseup", function mouseup() {
          disconnect(window, "mouseup", mouseup);
    ...
    function endOperation() {
      ...
      if (maxLineChanged && !options.lineWrapping) {
        ...
        sizer.style.minWidth = (width + 3) + "px";
  ...
  CodeMirror.fromTextArea = function(textarea, options) {
    ...
    if (textarea.form) {
      ...
      connect(textarea.form, "submit", save);
    ...
    instance.toTextArea = function() {
      ...
      if (textarea.form) {
        disconnect(textarea.form, "submit", save);
        if (typeof textarea.form.submit == "function")
          textarea.form.submit = realSubmit;
      }
    };
  ...
  function connect(emitter, type, f) {
  ...
  function disconnect(emitter, type, f) {
  ...
  CodeMirror.connect = connect; CodeMirror.disconnect = disconnect; CodeMirror.signal = signal;
  ...
  function scrollbarWidth(measure) {
    if (knownScrollbarWidth) return knownScrollbarWidth;
