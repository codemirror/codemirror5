...
window.CodeMirror = (function() {
  ...
  function updateSelectionCursor(cm) {
    if (cm.options.readOnly === "nocursor") {
        cm.display.cursor.style.display = "none";
        return;
    }
  ...
  function restartBlink(cm) {
    if (!cm.state.focused || cm.options.readOnly == "nocursor") return;
    var display = cm.display;
  ...
  function focusInput(cm) {
    if (cm.options.readOnly != "nofocus" && (!mobile || document.activeElement != cm.display.input))
  ...
  function onFocus(cm) {
    if (cm.options.readOnly == "nofocus") return;
    if (!cm.state.focused) {
      signal(cm, "focus", cm);
  ...
  option("readOnly", false, function(cm, val) {
    if (val == "nofocus") {onBlur(cm); cm.display.input.blur();}
