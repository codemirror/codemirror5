...
//    inputDiv.style.width = "30em"; inputDiv.style.height = "5em"; input.rows = 3;
    ...
    function getStateBefore(n) {
      ...
      for (var search = n - 1, lim = n - 40;; --search) {
        if (search < 0) {state = mode.startState ? mode.startState() : true; break;}
        if (search < lim) return null;
        if (state = lines[search].stateAfter) {state = copyState(mode, state); break;}
      }
    ...
    function highlightWorker(start) {
      ...
      while (work.length) {
        ...
        if (state) state = copyState(mode, state);
          ...
          state = mode.startState ? mode.startState() : true;
